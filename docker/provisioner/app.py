"""iDeer Sandbox Provisioner Service.
...
K8S_NAMESPACE = os.environ.get("K8S_NAMESPACE", "ideer")
SANDBOX_IMAGE = os.environ.get(
    "SANDBOX_IMAGE",
    "enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest",
)
SKILLS_HOST_PATH = os.environ.get("SKILLS_HOST_PATH", "/skills")
THREADS_HOST_PATH = os.environ.get("THREADS_HOST_PATH", "/.ideer/threads")
...
app = FastAPI(title="iDeer Sandbox Provisioner", lifespan=lifespan)
...
def _build_volume_mounts(thread_id: str, user_id: str = DEFAULT_USER_ID) -> list[k8s_client.V1VolumeMount]:
    ...
    if USERDATA_PVC_NAME:
        userdata_mount.sub_path = f"ideer/users/{user_id}/threads/{thread_id}/user-data"
...
async def list_sandboxes():
    ...
    try:
        services = core_v1.list_namespaced_service(
            K8S_NAMESPACE,
            label_selector="app=ideer-sandbox",
        )
    except ApiException as exc:
        raise HTTPException(
