...
Communicate with a running iDeer instance via its HTTP API. iDeer is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

iDeer exposes two API surfaces behind an Nginx reverse proxy:
...
```bash
bash /path/to/skills/claude-to-ideer/scripts/chat.sh "Your question here"
```
...
- If health check fails, iDeer is not running. Inform the user they need to start it.
