...
description_zh: "当用户需要对某个主题的多个学术论文进行系统文献综述、调研或综合时使用本技能。也涵盖带注释的参考书目与跨论文比较。搜索 arXiv,并以 APA、IEEE 或 BibTeX 格式输出报告。不适用于单篇论文任务,评审单篇论文请使用 academic-paper-review。"
...
**Concurrency limit: at most 3 subagents per turn.** The iDeer runtime enforces `MAX_CONCURRENT_SUBAGENTS = 3` and will silently drop any extra dispatches in the same turn — the LLM will not be told this happened, so strictly follow the round strategy below.
...
- **arXiv only, by design**. This skill does not query Semantic Scholar, PubMed, or Google Scholar. arXiv covers the bulk of CS/ML/physics/math preprints, which is what iDeer users most often want to survey. Multi-source academic search belongs in a dedicated MCP server, not inside this skill.
