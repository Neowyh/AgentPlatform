...
COMPOSE_CMD="docker compose -p ideer-dev -f docker-compose-dev.yaml"
...
detect_sandbox_mode() {
    ...
    if [[ "$sandbox_use" == *"ideer.sandbox.local:LocalSandboxProvider"* ]]; then
        echo "local"
    elif [[ "$sandbox_use" == *"ideer.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
init() {
    echo "=========================================="
    echo "  iDeer Init — Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting iDeer Docker Development"
    echo "=========================================="
    echo ""
    ...
    # Set IDEER_ROOT for provisioner if not already set
    if [ -z "$IDEER_ROOT" ]; then
        export IDEER_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting IDEER_ROOT=$IDEER_ROOT${NC}"
        echo ""
    fi
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting iDeer.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
...
stop() {
    # IDEER_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" ideer-sandbox 2>/dev/null || true
...
restart() {
    echo "========================================"
    echo "  Restarting iDeer Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "iDeer Docker Management Script"
    echo ""
