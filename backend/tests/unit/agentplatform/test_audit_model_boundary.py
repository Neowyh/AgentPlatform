...
def test_resource_runtime_seam_preserves_legacy_implementation_identity() -> None:
    from app.agentplatform.resource_runtime import ResourceStorage
    from ideer.resources.storage import ResourceStorage as LegacyStorage

    assert ResourceStorage is LegacyStorage
...
def test_workflow_runtime_seam_preserves_store_identity() -> None:
    ...
    from ideer.workflows.v2.store import WorkflowV2Store as LegacyStore

    assert WorkflowV2Store is LegacyStore
