...
class TestStartRun:
    ...
    @pytest.mark.asyncio
    async def test_start_run_shared_agent_stamps_owner(self, mock_deps, tmp_path):
        """A shared agent owned by another user is resolved via resource_metadata
        and the owner id is stamped into the run config."""
        bridge, run_mgr, run_ctx, request = mock_deps

        record = MagicMock()
        record.run_id = "run-shared"
        record.task = None
        run_mgr.create_or_reject.return_value = record
        ...
        body = SimpleNamespace(
            ...
            interrupt_before=None,
            interrupt_after=None,
            context={"agent_name": "fault-zeroing"},
        )
        ...
        with (
            ...
            patch("app.gateway.services.get_run_context", return_value=run_ctx),
            patch("app.gateway.services.resolve_agent_factory") as mock_factory,
            patch("app.gateway.services.run_agent", new_callable=AsyncMock) as mock_run_agent,
            patch("app.gateway.services.get_app_config") as mock_app_config,
            patch(
                "ideer.config.agents_config.resolve_agent_dir",
                return_value=tmp_path / "users" / "user-1" / "agents" / "fault-zeroing",
            ),
            patch("app.gateway.services.ResourceMetadataStore") as mock_store_cls,
        ...
        config = mock_run_agent.call_args.kwargs["config"]
        assert config["context"]["agent_owner_id"] == "owner-1"
        assert config["configurable"]["agent_owner_id"] == "owner-1"

    @pytest.mark.asyncio
    async def test_start_run_shared_agent_denied_is_404_before_run_record(self, mock_deps, tmp_path):
        """A shared agent the runner cannot access 404s without creating a run record."""
        bridge, run_mgr, run_ctx, request = mock_deps
        ...
        body = SimpleNamespace(
            ...
            interrupt_before=None,
            interrupt_after=None,
            context={"agent_name": "private-agent"},
        )

        owner_dir = tmp_path / "users" / "owner-1" / "agents" / "private-agent"
        owner_dir.mkdir(parents=True)
        ...
        with (
            ...
            patch(
                "ideer.config.agents_config.resolve_agent_dir",
                return_value=tmp_path / "users" / "user-1" / "agents" / "private-agent",
            ),
            patch("app.gateway.services.ResourceMetadataStore") as mock_store_cls,
            patch("ideer.config.paths.get_paths") as mock_paths,
    ...
    @pytest.mark.asyncio
    async def test_start_run_own_agent_skips_owner_resolution(self, mock_deps, tmp_path):
        """An agent present in the runner's own directory gets no owner stamp."""
        bridge, run_mgr, run_ctx, request = mock_deps

        record = MagicMock()
        record.run_id = "run-own"
        record.task = None
        run_mgr.create_or_reject.return_value = record

        own_dir = tmp_path / "users" / "user-1" / "agents" / "my-agent"
        ...
        with (
            ...
            patch("app.gateway.services.run_agent", new_callable=AsyncMock) as mock_run_agent,
            patch("app.gateway.services.get_app_config") as mock_app_config,
            patch("ideer.config.agents_config.resolve_agent_dir", return_value=own_dir),
            patch("app.gateway.services.ResourceMetadataStore") as mock_store_cls,
        ):
            mock_factory.return_value = MagicMock()
            mock_app_config.return_value.get_model_config.return_value = None
        ...
        config = mock_run_agent.call_args.kwargs["config"]
        assert "agent_owner_id" not in config.get("context", {})
        assert "agent_owner_id" not in config.get("configurable", {})

    @pytest.mark.asyncio
    async def test_start_run_shared_agent_no_meta_falls_back(self, mock_deps, tmp_path):
        """Without resource_metadata the legacy behavior is preserved: no owner
        resolution, no 404, no stamp."""
        bridge, run_mgr, run_ctx, request = mock_deps

        record = MagicMock()
        record.run_id = "run-no-meta"
        record.task = None
        run_mgr.create_or_reject.return_value = record
        ...
        body = SimpleNamespace(
            ...
            interrupt_before=None,
            interrupt_after=None,
            context={"agent_name": "ghost-agent"},
        )
        ...
        with (
            ...
            patch(
                "ideer.config.agents_config.resolve_agent_dir",
                return_value=tmp_path / "users" / "user-1" / "agents" / "ghost-agent",
            ),
            patch("app.gateway.services.ResourceMetadataStore") as mock_store_cls,
        ):
            mock_factory.return_value = MagicMock()
