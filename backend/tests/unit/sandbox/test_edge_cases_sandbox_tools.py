"""Additional coverage tests for deerflow.sandbox.tools — targeting uncovered lines."""
...
class TestGetAcpWorkspaceHostPathEdge:
    ...
    def test_with_thread_id_exception(self):
        ...
        with patch("deerflow.config.paths.get_paths", side_effect=Exception("fail")):
            result = _get_acp_workspace_host_path("t1")
            assert result is None
...
class TestGetMcpAllowedPathsEdge:
    def test_no_filesystem_server(self):
        ...
        with patch("deerflow.config.extensions_config.get_extensions_config", return_value=mock_ext):
...
class TestMaskLocalPathsInOutputEdge:
    def test_with_acp_workspace_path(self):
        ...
        with patch("deerflow.sandbox.tools._get_skills_host_path", return_value=None):
            with patch("deerflow.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"):
                with patch("deerflow.sandbox.tools._extract_thread_id_from_thread_data", return_value="t1"):
                    with patch("deerflow.sandbox.tools._get_acp_workspace_host_path", return_value="/base/acp"):
...
class TestThreadActualToVirtualMappings:
    def test_basic(self):
        from deerflow.sandbox.tools import _thread_actual_to_virtual_mappings
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=True):
                with patch("deerflow.sandbox.tools.is_host_bash_allowed", return_value=True):
                    with patch("deerflow.sandbox.tools.ensure_thread_directories_exist"):
                        with patch("deerflow.sandbox.tools.get_thread_data", return_value=_make_thread_data()):
                            with patch("deerflow.sandbox.tools.validate_local_bash_command_paths"):
                                with patch("deerflow.sandbox.tools.replace_virtual_paths_in_command", return_value="ls"):
                                    with patch("deerflow.sandbox.tools._apply_cwd_prefix", return_value="ls"):
                ...
                with patch("deerflow.sandbox.tools.validate_local_tool_path"):
                    with patch("deerflow.sandbox.tools._is_skills_path", return_value=False):
                        with patch("deerflow.sandbox.tools._is_acp_workspace_path", return_value=True):
                            with patch("deerflow.sandbox.tools._resolve_acp_workspace_path", return_value="/host/acp"):
                                with patch("deerflow.sandbox.tools.get_thread_data", return_value=_make_thread_data()):
                                    with patch("deerflow.sandbox.tools.mask_local_paths_in_output", return_value="file.py"):
                    ...
                    with patch("deerflow.sandbox.tools._is_skills_path", return_value=False):
                        with patch("deerflow.sandbox.tools._is_acp_workspace_path", return_value=False):
                            with patch("deerflow.sandbox.tools._is_custom_mount_path", return_value=True):
                                with patch("deerflow.sandbox.tools.get_thread_data", return_value=_make_thread_data()):
                                    with patch("deerflow.sandbox.tools.mask_local_paths_in_output", return_value="data.txt"):
                                        with patch("deerflow.sandbox.tools.get_app_config") as mock_cfg:
                                            mock_cfg.return_value.sandbox.ls_output_max_chars = 20000
            ...
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=True):
                with patch("deerflow.sandbox.tools.validate_local_tool_path"):
                    with patch("deerflow.sandbox.tools._is_custom_mount_path", return_value=True):
                        with patch("deerflow.sandbox.tools.get_thread_data", return_value=_make_thread_data()):
                            with patch("deerflow.sandbox.tools.get_file_operation_lock", return_value=mock_lock):
                                result = str_replace_tool.func(runtime, "/mnt/mount/file.txt", "hello", "bye", "test")
                                assert result == "OK"
