...
class TestHostPathMasking:
    ...
    def test_host_skills_paths_masked(self, thread_data):
        ...
        with (
            ...
            patch("deerflow.sandbox.tools._get_skills_host_path", return_value="/opt/app/skills"),
        ):
    ...
    def test_host_acp_workspace_paths_masked(self, thread_data):
        ...
        with patch("deerflow.sandbox.tools._get_acp_workspace_host_path", return_value="/home/user/.ideer/acp-workspace"):
...
class TestBashCommandAuthorization:
    ...
    def test_bash_allowed_when_host_bash_enabled(self, monkeypatch):
        ...
        mock_sandbox = SimpleNamespace(execute_command=lambda cmd, env=None, timeout=None: "hello\n")
        ...
        monkeypatch.setattr("deerflow.sandbox.tools.ensure_sandbox_initialized", lambda rt: mock_sandbox)
        monkeypatch.setattr("deerflow.sandbox.tools.ensure_thread_directories_exist", lambda rt: None)
        monkeypatch.setattr("deerflow.sandbox.tools.is_host_bash_allowed", lambda: True)
        monkeypatch.setattr("deerflow.sandbox.tools.validate_local_bash_command_paths", lambda cmd, td: None)
        monkeypatch.setattr("deerflow.sandbox.tools.replace_virtual_paths_in_command", lambda cmd, td: cmd)
        monkeypatch.setattr("deerflow.sandbox.tools._apply_cwd_prefix", lambda cmd, td: cmd)
        monkeypatch.setattr("deerflow.sandbox.tools.mask_local_paths_in_output", lambda out, td: out)
        monkeypatch.setattr("deerflow.sandbox.tools._truncate_bash_output", lambda out, mx: out)
        monkeypatch.setattr("deerflow.sandbox.tools.get_app_config", lambda: SimpleNamespace(sandbox=SimpleNamespace(bash_output_max_chars=20000)))
...
class TestFileOperationLock:
    ...
    def test_lock_prevents_concurrent_write_read_race(self, monkeypatch):
        ...
        monkeypatch.setattr("deerflow.sandbox.tools.ensure_sandbox_initialized", lambda rt: sandbox)
        monkeypatch.setattr("deerflow.sandbox.tools.ensure_thread_directories_exist", lambda rt: None)
        monkeypatch.setattr("deerflow.sandbox.tools.is_local_sandbox", lambda rt: False)
...
class TestCommandTimeout:
    ...
    def test_execute_command_has_timeout(self, sandbox_with_mappings):
        ...
        import deerflow.sandbox.local.local_sandbox as local_sandbox_module

        captured: dict = {}

        def _fake_run_posix(args, timeout, env=None):
            captured["timeout"] = timeout
            return ("ok\n", "", 0, False)

        with patch.object(LocalSandbox, "_run_posix_command", staticmethod(_fake_run_posix)):
            sandbox_with_mappings.execute_command("echo ok")

        assert captured["timeout"] > 0
        assert captured["timeout"] <= 600
        assert local_sandbox_module.DEFAULT_COMMAND_TIMEOUT_SECONDS <= 600
    ...
    def test_execute_command_capture_output(self, sandbox_with_mappings):
        """stdout/stderr must be redirected into capture pipes, not inherited."""
        fake_process = MagicMock()
        fake_process.pid = 12345
        fake_process.returncode = 0
        with patch("subprocess.Popen", return_value=fake_process) as mock_popen:
...
class TestSecurityConfigDefaults:
    ...
    def test_allow_host_bash_defaults_false(self):
        ...
        config = SandboxConfig(use="deerflow.sandbox.local:LocalSandboxProvider")
