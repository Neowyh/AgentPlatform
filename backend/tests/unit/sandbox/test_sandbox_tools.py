"""Tests for deerflow.sandbox.tools — sandbox tool functions and path utilities."""
...
class TestGetSkillsContainerPath:
    def test_default(self):
        ...
        # module (deerflow.config) rather than the sandbox.tools namespace.
        with patch("deerflow.config.get_app_config", side_effect=Exception("no config")):
...
class TestPathChecks:
    def test_is_skills_path(self):
        from deerflow.sandbox.tools import _is_skills_path
    ...
    def test_is_acp_workspace_path(self):
        from deerflow.sandbox.tools import _is_acp_workspace_path
...
class TestValidateLocalToolPath:
    ...
    def test_acp_workspace_read_only(self):
        ...
        td = _make_thread_data()
        with patch("deerflow.sandbox.tools._is_skills_path", return_value=False):
            with patch("deerflow.sandbox.tools._is_acp_workspace_path", return_value=True):
    ...
    def test_unknown_path_raises(self):
        ...
        td = _make_thread_data()
        with patch("deerflow.sandbox.tools._is_skills_path", return_value=False):
            with patch("deerflow.sandbox.tools._is_acp_workspace_path", return_value=False):
                with patch("deerflow.sandbox.tools._is_custom_mount_path", return_value=False):
...
class TestMaskLocalPathsInOutput:
    ...
    def test_masks_skills_path(self):
        ...
        td = _make_thread_data()
        with patch("deerflow.sandbox.tools._get_skills_host_path", return_value="/opt/skills"):
            with patch("deerflow.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"):
...
class TestSanitizeError:
    ...
    def test_with_local_runtime(self):
        ...
        with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=True):
            with patch("deerflow.sandbox.tools.get_thread_data", return_value=td):
                with patch("deerflow.sandbox.tools.mask_local_paths_in_output", return_value="masked"):
...
class TestSandboxFromRuntime:
    ...
    def test_sandbox_not_found(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider") as mock_provider:
...
class TestShellTokenHelpers:
    ...
    def test_unsafe_cwd_targets_raise(self):
        from deerflow.sandbox.tools import _validate_local_bash_cwd_target, _validate_local_bash_shell_tokens

        # The old boolean helper was replaced upstream by a raising validator.
        for target in (None, "-", "~", "/"):
            with pytest.raises(PermissionError):
                _validate_local_bash_cwd_target("cd", target, [])

        # Relative .. escapes are rejected by the token-level traversal gate.
        with pytest.raises(PermissionError):
            _validate_local_bash_shell_tokens("cd ../etc", [])

        # Safe relative targets pass through without raising.
        _validate_local_bash_cwd_target("cd", "./safe", [])
...
class TestCustomMounts:
    ...
    def test_is_custom_mount_path_no_mounts(self):
        ...
        with patch("deerflow.sandbox.tools._get_custom_mounts", return_value=[]):
...
class TestValidateLocalBashCwdTarget:
    ...
    def test_absolute_allowed(self):
        ...
        with patch("deerflow.sandbox.tools._is_allowed_local_bash_absolute_path", return_value=True):
    ...
    def test_absolute_disallowed(self):
        ...
        with pytest.raises(PermissionError):
            with patch("deerflow.sandbox.tools._is_allowed_local_bash_absolute_path", return_value=False):
...
class TestAllowedPathsExtended:
    ...
    def test_custom_mount_path(self):
        ...
        with patch("deerflow.sandbox.tools._is_skills_path", return_value=False):
            with patch("deerflow.sandbox.tools._is_acp_workspace_path", return_value=False):
                with patch("deerflow.sandbox.tools._is_custom_mount_path", return_value=True):
...
class TestValidateLocalToolPathExtra:
    ...
    def test_custom_mount_read_only_raises(self):
        ...
        with patch("deerflow.sandbox.tools._is_skills_path", return_value=False):
            with patch("deerflow.sandbox.tools._is_acp_workspace_path", return_value=False):
                with patch("deerflow.sandbox.tools._is_custom_mount_path", return_value=True):
                    with patch("deerflow.sandbox.tools._get_custom_mount_for_path", return_value=mock_mount):
...
class TestEnsureSandboxInitializedExtra:
    ...
    def test_lazy_acquisition(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            result = ensure_sandbox_initialized(runtime)
            assert result == mock_sandbox
            mock_provider.acquire.assert_called_once_with("t1", user_id="test-user-autouse")
...
class TestTruncateBashOutputExtra:
    ...
    def test_very_small_max(self):
        from deerflow.sandbox.tools import _BASH_OUTPUT_MIN_LIMIT_CHARS, _truncate_bash_output

        long = "x" * 100
        result = _truncate_bash_output(long, 5)
        # Upstream applies a 32-char floor so a trailing exit marker stays
        # preservable: configured limits in 1..31 yield up to 32 chars.
        assert len(result) <= max(5, _BASH_OUTPUT_MIN_LIMIT_CHARS)
...
class TestMaxResultsExtra:
    ...
    def test_resolve_max_results(self):
        from deerflow.sandbox.tools import _resolve_max_results

        with patch("deerflow.sandbox.tools._get_tool_config_int", return_value=300):
    ...
    def test_get_tool_config_int_exception(self):
        ...
        with patch("deerflow.sandbox.tools.get_app_config", side_effect=Exception("no")):
...
class TestGetAcpWorkspaceHostPath:
    def test_with_thread_id_not_found(self):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            with patch("deerflow.runtime.user_context.get_effective_user_id", return_value="u1"):
...
class TestCdTarget:
    ...
    def test_unsafe_cwd_targets_raise(self):
        ...
        # The old boolean helper was replaced upstream by a raising validator.
        for target in (None, "-", "$HOME", "~", "/tmp"):
            with pytest.raises(PermissionError):
                _validate_local_bash_cwd_target("cd", target, [])

        # Relative .. escapes are rejected by the token-level traversal gate.
        with pytest.raises(PermissionError):
            _validate_local_bash_shell_tokens("cd ../escape", [])
...
class TestBashTool:
    def test_local_sandbox_host_bash_disabled(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=True):
                with patch("deerflow.sandbox.tools.is_host_bash_allowed", return_value=False):
    ...
    def test_local_sandbox_success(self, tmp_path):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=True):
                with patch("deerflow.sandbox.tools.is_host_bash_allowed", return_value=True):
                    with patch("deerflow.sandbox.tools.get_app_config") as mock_cfg:
...
class TestWriteFileTool:
    def test_success(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=False):
                with patch("deerflow.sandbox.tools.get_file_operation_lock", return_value=mock_lock):
...
class TestStrReplaceTool:
    def test_success(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=False):
                with patch("deerflow.sandbox.tools.get_file_operation_lock", return_value=mock_lock):
                    result = str_replace_tool.func(runtime, "/tmp/test.txt", "hello", "goodbye", "test")
                    assert result == "OK"
    ...
    def test_empty_file(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=False):
                with patch("deerflow.sandbox.tools.get_file_operation_lock", return_value=mock_lock):
                    # Upstream reports the graceful not-found error for empty
                    # content instead of the legacy silent OK.
                    result = str_replace_tool.func(runtime, "/tmp/test.txt", "old", "new", "test")
                    assert "not found" in result.lower()
    ...
    def test_replace_all(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=False):
                with patch("deerflow.sandbox.tools.get_file_operation_lock", return_value=mock_lock):
                    result = str_replace_tool.func(runtime, "/tmp/test.txt", "aaa", "ccc", "test", replace_all=True)
                    assert result == "OK"
...
class TestRunSyncToolAfterAsyncSandboxInit:
    ...
    async def test_none_func(self):
        ...
        mock_provider = MagicMock()
        mock_provider.get.return_value = mock_sandbox
        with patch("deerflow.sandbox.tools.ensure_sandbox_initialized_async", return_value=mock_sandbox):
            result = await _run_sync_tool_after_async_sandbox_init(None, runtime)
...
class TestAsyncToolWrappers:
    ...
    async def test_bash_tool_async(self):
        ...
        runtime = _make_runtime()
        with patch("deerflow.sandbox.tools._run_sync_tool_after_async_sandbox_init", new_callable=AsyncMock, return_value="ok"):
    ...
    async def test_ls_tool_async(self):
        ...
        runtime = _make_runtime()
        with patch("deerflow.sandbox.tools._run_sync_tool_after_async_sandbox_init", new_callable=AsyncMock, return_value="file.txt"):
    ...
    async def test_glob_tool_async(self):
        ...
        runtime = _make_runtime()
        with patch("deerflow.sandbox.tools._run_sync_tool_after_async_sandbox_init", new_callable=AsyncMock, return_value="Found 1 paths"):
    ...
    async def test_grep_tool_async(self):
        ...
        runtime = _make_runtime()
        with patch("deerflow.sandbox.tools._run_sync_tool_after_async_sandbox_init", new_callable=AsyncMock, return_value="Found 1 matches"):
    ...
    async def test_read_file_tool_async(self):
        ...
        runtime = _make_runtime()
        with patch("deerflow.sandbox.tools._run_sync_tool_after_async_sandbox_init", new_callable=AsyncMock, return_value="content"):
...
class TestResolveLocalReadPath:
    def test_skills_path(self):
        ...
        td = _make_thread_data()
        with patch("deerflow.sandbox.tools._is_skills_path", return_value=True):
            # Upstream returns mounted paths unchanged: the sandbox's
            # PathMapping resolves them at execution time.
    ...
    def test_acp_path(self):
        ...
        with patch("deerflow.sandbox.tools._is_skills_path", return_value=False):
            with patch("deerflow.sandbox.tools._is_acp_workspace_path", return_value=True):
                # Mounted ACP workspace paths are returned unchanged as well.
    ...
    def test_user_data_path(self):
        ...
        with patch("deerflow.sandbox.tools._is_skills_path", return_value=False):
            with patch("deerflow.sandbox.tools._is_acp_workspace_path", return_value=False):
                with patch("deerflow.sandbox.tools._resolve_and_validate_user_data_path", return_value="/resolved"):
...
class TestResolveSkillsPath:
    def test_root_path(self):
        from deerflow.sandbox.tools import _resolve_skills_path
    ...
    def test_no_host_path_raises(self):
        ...
        with patch("deerflow.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"):
            with patch("deerflow.sandbox.tools._get_skills_host_path", return_value=None):
...
class TestGetCustomMountsExtended:
    ...
    def test_no_sandbox_config(self):
        ...
        try:
            # The function imports get_app_config locally, so patch the source
            # module (deerflow.config) rather than the sandbox.tools namespace.
            with patch("deerflow.config.get_app_config") as mock_cfg:
...
class TestGetCustomMountForPath:
    def test_longest_prefix_wins(self):
        ...
        with patch("deerflow.sandbox.tools._get_custom_mounts", return_value=[mount1, mount2]):
...
class TestResolveAcpWorkspacePath:
    def test_root_path(self):
        ...
        with patch("deerflow.sandbox.tools._get_acp_workspace_host_path", return_value="/base/acp"):
...
class TestGetMcpAllowedPaths:
    def test_no_extensions_config(self):
        ...
        with patch("deerflow.config.extensions_config.get_extensions_config", side_effect=Exception("no config")):
...
class TestResolveMaxResultsExtended:
    def test_respects_lower_bound(self):
        ...
        with patch("deerflow.sandbox.tools._get_tool_config_int", return_value=50):
    ...
    def test_respects_upper_bound(self):
        ...
        with patch("deerflow.sandbox.tools._get_tool_config_int", return_value=1000):
            result = _resolve_max_results("glob", 200, default=100, upper_bound=500)
            assert result == 200
...
class TestThreadVirtualToActualMappings:
    def test_all_paths(self):
        from deerflow.sandbox.tools import _thread_virtual_to_actual_mappings
...
class TestValidateLocalBashRootPathArgs:
    def test_slash_only_raises(self):
        from deerflow.sandbox.tools import _validate_local_bash_root_path_args
...
class TestValidateLocalBashShellTokensTraversal:
    ...
    def test_dotdot_in_middle(self):
        ...
        with pytest.raises(PermissionError):
            _validate_local_bash_shell_tokens("cd foo/../bar", [])
...
class TestLsToolExtended:
    def test_local_sandbox_with_skills_path(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=True):
                with patch("deerflow.sandbox.tools.validate_local_tool_path"):
                    with patch("deerflow.sandbox.tools._is_skills_path", return_value=True):
                        with patch("deerflow.sandbox.tools._resolve_skills_path", return_value="/host/skills"):
                            with patch("deerflow.sandbox.tools.get_thread_data", return_value=_make_thread_data()):
                                with patch("deerflow.sandbox.tools.mask_local_paths_in_output", return_value="SKILL.md"):
                                    with patch("deerflow.sandbox.tools.get_app_config") as mock_cfg:
                                        mock_cfg.return_value.sandbox.ls_output_max_chars = 20000
...
class TestStrReplaceToolExtended:
    ...
    def test_replace_all_flag(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            with patch("deerflow.sandbox.tools.is_local_sandbox", return_value=False):
                with patch("deerflow.sandbox.tools.get_file_operation_lock", return_value=mock_lock):
                    result = str_replace_tool.func(runtime, "/tmp/test", "aaa", "ccc", "test", replace_all=True)
                    assert result == "OK"
...
class TestFormatWriteFileErrorExtended:
    def test_with_runtime(self):
        ...
        td = _make_thread_data()
        runtime = _make_runtime(thread_data=td)
        with patch("deerflow.sandbox.tools._sanitize_error", return_value="sanitized"):
...
class TestEnsureSandboxInitializedExtended:
    def test_thread_id_from_config(self):
        ...
        with patch("deerflow.sandbox.tools.get_sandbox_provider", return_value=mock_provider):
            result = ensure_sandbox_initialized(runtime)
            assert result is mock_sandbox
            mock_provider.acquire.assert_called_once_with("from_config", user_id="test-user-autouse")
...
class TestRunSyncToolAfterAsyncSandboxInitExtended:
    def test_success_with_args(self):
        ...
        async def go():
            runtime = _make_runtime()
            sync_func = MagicMock(return_value="result")
            with patch("deerflow.sandbox.tools.ensure_sandbox_initialized_async", new_callable=AsyncMock):
...
class TestReplaceVirtualPathsInCommandExtended:
    def test_skills_path_left_for_execution_time_resolution(self):
        from deerflow.sandbox.tools import replace_virtual_paths_in_command

        # Upstream no longer rewrites /mnt/skills in the command string:
        # LocalSandbox resolves mounted paths via PathMapping at execution
        # time so the mapping's user_id wins over the contextvar.
        td = _make_thread_data()
        result = replace_virtual_paths_in_command("cat /mnt/skills/test.md", td)
        assert result == "cat /mnt/skills/test.md"

    def test_acp_workspace_left_for_execution_time_resolution(self):
        from deerflow.sandbox.tools import replace_virtual_paths_in_command
