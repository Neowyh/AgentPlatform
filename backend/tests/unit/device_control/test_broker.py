...
async def test_signed_echo_task_has_ack_progress_result_receipt_lifecycle() -> None:
    ...
    for message_type, payload, expected in (
        ...
        (
            ...
            {"receipt": {"status": "completed", "task_id": record.task_id}},
...
@pytest.mark.asyncio
async def test_invalidate_closes_session_and_fails_pending_tasks_closed() -> None:
    broker = DeviceBroker()
    device_key = Ed25519PrivateKey.generate()
    socket = FakeWebSocket()
    connection = DeviceConnection(
    ...
    record = await broker.send_echo_task(
        ...
        run_id="run-revoked",
        tool_call_id="tool-revoked",
        value="hello",
    )

    await broker.invalidate(connection.device_id, reason="device revoked")

    assert socket.closed is True
    assert connection.tasks_allowed is False
    assert record.status is TaskStatus.DEVICE_OFFLINE
    rejected = await broker.send_echo_task(
        device_id=connection.device_id,
        run_id="run-after-revoke",
        tool_call_id="tool-after-revoke",
        value="hello",
    )
...
@pytest.mark.asyncio
async def test_capability_update_keeps_only_descriptors_for_announced_tools() -> None:
    broker = DeviceBroker()
    device_key = Ed25519PrivateKey.generate()
    connection = DeviceConnection(
        "device-schema",
    ...
    message = sign_envelope(
        ...
        payload={
            "capabilities": ["local.mcp.fs.read_file"],
            "tool_descriptors": {
...
@pytest.mark.asyncio
async def test_unannounced_local_capability_is_rejected() -> None:
    broker = DeviceBroker()
    device_key = Ed25519PrivateKey.generate()
    connection = DeviceConnection(
        "device-no-cap",
    ...
    record = await broker.send_task(
        device_id="device-no-cap",
        operation="local.files.read",
        path="/tmp/a",
        run_id="run-no-cap",
        tool_call_id="tool-no-cap",
    )
...
async def test_consent_required_is_recorded_and_decision_is_forwarded() -> None:
    ...
    connection = DeviceConnection(
        ...
        device_key.public_key(),
        socket,
        capabilities=frozenset({"local.files.write"}),
    )
    await broker.attach(connection)
    record = await broker.send_task(
...
async def test_error_receipt_preserves_denied_status() -> None:
    ...
    record = await broker.send_echo_task(
        device_id=connection.device_id,
        run_id="run-error-denied",
        tool_call_id="tool-error-denied",
        value="x",
    )
    message = sign_envelope(
        private_key=device_key,
...
async def test_connected_device_without_requested_capability_rejects_local_task() -> (
    ...
    connection = DeviceConnection(
        ...
        device_key.public_key(),
        FakeWebSocket(),
        capabilities=frozenset({"local.files.read"}),
    )
    await broker.attach(connection)
