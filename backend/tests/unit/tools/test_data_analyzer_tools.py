...
from app.agentplatform.community.data_analyzer.tools import (
    _MAX_FILE_SIZE,
    _MAX_MEMORY_BYTES,
    _MAX_OUTPUT_CHARS,
    _MAX_ROWS,
...
class TestCheckPandas:
    ...
    def test_returns_error_when_pandas_none(self):
        with patch("app.agentplatform.community.data_analyzer.tools.pd", None):
...
class TestReadFile:
    ...
    def test_file_too_large(self):
        ...
        try:
            with patch(
                "app.agentplatform.community.data_analyzer.tools.os.path.getsize",
    ...
    def test_json_chunked_fallback(self):
        ...
        try:
            ...
            with patch("app.agentplatform.community.data_analyzer.tools.pd.read_json", side_effect=mock_read_json):
                with patch("app.agentplatform.community.data_analyzer.tools.pd", pd):
            ...
            with patch("app.agentplatform.community.data_analyzer.tools.pd") as mock_pd:
    ...
    def test_generic_exception_during_read(self):
        path = _tmp_csv("a,b\n1,2\n")
        try:
            with patch("app.agentplatform.community.data_analyzer.tools.pd.read_csv", side_effect=RuntimeError("boom")):
                df, err = _read_file(path)
                assert df is None
    ...
    def test_decompression_bomb_protection(self):
        ...
        try:
            ...
            with patch("app.agentplatform.community.data_analyzer.tools.pd.read_csv", return_value=fake_df):
    ...
    def test_row_truncation(self):
        ...
        try:
            ...
            with patch("app.agentplatform.community.data_analyzer.tools.pd.read_csv", return_value=big_df):
...
class TestDataAnalyzerTool:
    ...
    def test_analysis_exception_returns_error(self):
        ...
        try:
            with patch(
                "app.agentplatform.community.data_analyzer.tools._analyze_summary",
