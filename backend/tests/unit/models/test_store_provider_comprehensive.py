"""Tests for deerflow.runtime.store.provider — sync store factory (gap coverage)."""
...
class TestSyncStoreCm:
    ...
    def test_sqlite_type_success(self):
        ...
        with patch.dict("sys.modules", {"langgraph.store.sqlite": mock_module}):
            with patch("deerflow.runtime.store.provider.resolve_sqlite_conn_str", return_value="/resolved.db"):
                with patch("deerflow.runtime.store.provider.ensure_sqlite_parent_dir"):
                    with _sync_store_cm(config) as store:
    ...
    def test_sqlite_default_conn_string(self):
        ...
        with patch.dict("sys.modules", {"langgraph.store.sqlite": mock_module}):
            with patch("deerflow.runtime.store.provider.resolve_sqlite_conn_str", return_value="/default.db"):
                with patch("deerflow.runtime.store.provider.ensure_sqlite_parent_dir"):
    ...
    def test_postgres_type_success(self):
        ...
        config = SimpleNamespace(type="postgres", connection_string="postgresql://localhost/db", postgres_schema=None)
...
class TestGetStore:
    ...
    def test_no_config_returns_in_memory(self):
        ...
        with patch("deerflow.config.checkpointer_config.get_checkpointer_config", return_value=None):
            with patch("deerflow.config.app_config._app_config", None):
    ...
    def test_with_checkpointer_config(self):
        ...
        config = SimpleNamespace(type="memory", connection_string=None)
        with patch("deerflow.config.checkpointer_config.get_checkpointer_config", return_value=config):
            store = get_store()
            assert store is not None
...
class TestResetStore:
    ...
    def test_closes_context_manager(self):
        ...
        mock_ctx = MagicMock()
        with patch("deerflow.runtime.store.provider._store_ctx", mock_ctx):
            with patch("deerflow.runtime.store.provider._store", MagicMock()):
                reset_store()
...
class TestStoreContext:
    def test_no_checkpointer_returns_in_memory(self):
        ...
        app_config = SimpleNamespace(checkpointer=None, database=None)
