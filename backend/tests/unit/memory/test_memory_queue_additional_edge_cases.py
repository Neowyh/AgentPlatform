...
def test_process_queue_continues_on_updater_exception():
    ...
    from app.agentplatform.legacy.memory.queue import ConversationContext
...
def test_reset_timer_sets_timer():
    ...
    queue = MemoryUpdateQueue()
    with (
        patch("app.agentplatform.legacy.memory.queue.get_memory_config", return_value=_memory_config(debounce_seconds=15)),
