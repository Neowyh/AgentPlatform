...
def test_repository_bundled_manifest_has_unique_stable_ids_and_existing_sources() -> None:
    ...
    assert len(manifest.resources) == 17
...
async def test_repository_bundle_can_be_seeded_offline_as_one_complete_set(
    ...
    async with factory() as session:
        assert await session.scalar(select(func.count()).select_from(Resource)) == 17
        assert await session.scalar(select(func.count()).select_from(ResourceVersion)) == 17
    await engine.dispose()
