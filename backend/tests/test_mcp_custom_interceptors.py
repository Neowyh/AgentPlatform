...
def test_custom_interceptor_loaded_and_appended():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("ideer.mcp.tools.resolve_variable", return_value=fake_builder),
    ):
...
def test_multiple_custom_interceptors():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("ideer.mcp.tools.resolve_variable", side_effect=lambda path: builders[path]),
    ):
        asyncio.run(get_mcp_tools())
...
def test_custom_interceptor_builder_returning_none_is_skipped():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("ideer.mcp.tools.resolve_variable", return_value=lambda: None),
    ):
        asyncio.run(get_mcp_tools())
...
def test_custom_interceptor_resolve_error_logs_warning_and_continues():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("ideer.mcp.tools.resolve_variable", side_effect=ImportError("no such module")),
        patch("ideer.mcp.tools.logger.warning") as mock_warn,
    ):
...
def test_custom_interceptor_builder_exception_logs_warning_and_continues():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("ideer.mcp.tools.resolve_variable", return_value=exploding_builder),
        patch("ideer.mcp.tools.logger.warning") as mock_warn,
    ):
        tools = asyncio.run(get_mcp_tools())
...
def test_custom_interceptor_coexists_with_oauth_interceptor():
    ...
    with (
        ...
        patch("ideer.mcp.tools.resolve_variable", return_value=lambda: custom_fn),
    ):
        asyncio.run(get_mcp_tools())
...
def test_mcp_interceptors_single_string_is_normalized():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("ideer.mcp.tools.resolve_variable", return_value=lambda: fake_interceptor),
    ):
        asyncio.run(get_mcp_tools())
...
def test_custom_interceptor_non_callable_return_logs_warning():
    ...
    with (
        ...
        p["oauth_headers"],
        p["oauth_interceptor"],
        patch("ideer.mcp.tools.resolve_variable", return_value=lambda: "not_a_callable"),
        patch("ideer.mcp.tools.logger.warning") as mock_warn,
    ):
        asyncio.run(get_mcp_tools())
