...
class TestCheckResourceModify:
    """Lines 39, 41: _check_resource_modify error paths."""

    def test_raises_401_when_no_user(self):
        from app.gateway.routers.agents import _check_resource_modify

        with pytest.raises(HTTPException) as excinfo:
            _check_resource_modify("owner-1", None, None)
        assert excinfo.value.status_code == 401

    def test_raises_403_when_not_permitted(self):
        from app.gateway.routers.agents import _check_resource_modify

        mock_user = MagicMock()
        with patch("app.gateway.routers.agents.check_resource_modify", return_value=False):
            with pytest.raises(HTTPException) as excinfo:
                _check_resource_modify("other-owner", None, mock_user)
    ...
    def test_passes_when_permitted(self):
        ...
        mock_user = MagicMock()
        with patch("app.gateway.routers.agents.check_resource_modify", return_value=True):
            # Should not raise
            _check_resource_modify("owner-1", None, mock_user)


class TestCanSetVisibility:
    """Lines 53, 56, 58: _can_set_visibility branches."""

    def test_no_user_always_allowed(self):
        from app.gateway.routers.agents import _can_set_visibility
    ...
    def test_public_requires_super_admin(self):
        ...
        assert _can_set_visibility("public", super_admin) is True

        user = MagicMock()
        user.role = UserRole.USER
        assert _can_set_visibility("public", user) is False

        dept_admin = MagicMock()
        dept_admin.role = UserRole.DEPARTMENT_ADMIN
        assert _can_set_visibility("public", dept_admin) is False

    def test_department_requires_admin_or_above(self):
        from app.gateway.routers.agents import _can_set_visibility
        from ideer.persistence.models.user import UserRole

        dept_admin = MagicMock()
        dept_admin.role = UserRole.DEPARTMENT_ADMIN
        assert _can_set_visibility("department", dept_admin) is True

        super_admin = MagicMock()
        super_admin.role = UserRole.SUPER_ADMIN
        assert _can_set_visibility("department", super_admin) is True
...
class TestIsVisibleToUser:
    """Lines 187-196: _is_visible_to_user branches."""

    def test_public_visible_to_everyone(self):
        from app.gateway.routers.agents import _is_visible_to_user
        from ideer.persistence.models.user import UserRole
        ...
        assert _is_visible_to_user("public", "other-owner", "other-dept", user) is True
    ...
    def test_super_admin_sees_everything(self):
        ...
        assert _is_visible_to_user("private", "other-owner", "other-dept", admin) is True
    ...
    def test_department_visibility_same_dept(self):
        ...
        user.id = "user-1"
        user.department_id = "dept-1"
        assert _is_visible_to_user("department", "other-owner", "dept-1", user) is True
        assert _is_visible_to_user("department", "other-owner", "dept-2", user) is False

    def test_department_admin_sees_dept_resources(self):
        from app.gateway.routers.agents import _is_visible_to_user
        from ideer.persistence.models.user import UserRole

        admin = MagicMock()
        admin.role = UserRole.DEPARTMENT_ADMIN
        admin.id = "admin-1"
        admin.department_id = "dept-1"
        # Department admin can see resources in their own department
        assert _is_visible_to_user("private", "other-owner", "dept-1", admin) is True
        # But not in other departments
        assert _is_visible_to_user("private", "other-owner", "dept-2", admin) is False

    def test_non_visible_private_user(self):
        from app.gateway.routers.agents import _is_visible_to_user
        from ideer.persistence.models.user import UserRole
        ...
        user.id = "user-1"
        user.department_id = "dept-1"
        assert _is_visible_to_user("private", "other-owner", "other-dept", user) is False
