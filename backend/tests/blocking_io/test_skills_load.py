...
def _real_subagent_executor() -> Iterator[type]:
    ...
    original_executor = sys.modules.get("ideer.subagents.executor", _MISSING)
    parent_module = sys.modules.get("ideer.subagents")
    original_parent_executor = getattr(parent_module, "executor", _MISSING) if parent_module is not None else _MISSING

    sys.modules.pop("ideer.subagents.executor", None)
    for name in _EXECUTOR_IMPORT_MOCKS:
        sys.modules[name] = MagicMock()

    try:
        executor_module = importlib.import_module("ideer.subagents.executor")
