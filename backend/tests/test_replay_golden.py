...
def test_replay_write_read_file_ultra_matches_golden(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("IDEER_REPLAY_FIXTURE", str(fixture_path))

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(build_config_yaml(model_block=REPLAY_MODEL_BLOCK, home=home), encoding="utf-8")
    monkeypatch.setenv("IDEER_CONFIG_PATH", str(cfg_path))
    monkeypatch.setenv("IDEER_EXTENSIONS_CONFIG_PATH", str(prepare_hermetic_extras(home)))
    ...
    #   IDEER_WRITE_REPLAY_GOLDEN=1 uv run pytest tests/test_replay_golden.py
    if os.environ.get("IDEER_WRITE_REPLAY_GOLDEN"):
