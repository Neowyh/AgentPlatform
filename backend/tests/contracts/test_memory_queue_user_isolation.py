...
from ideer.agents.memory.queue import ConversationContext, MemoryUpdateQueue
...
def test_queue_add_stores_user_id():
    q = MemoryUpdateQueue()
    with patch("ideer.agents.memory.queue.get_memory_config", return_value=MemoryConfig(enabled=True)), patch.object(q, "_reset_timer"):
...
def test_add_nowait_keeps_different_users_separate():
    ...
    with (
        patch("ideer.agents.memory.queue.get_memory_config", return_value=MemoryConfig(enabled=True)),
