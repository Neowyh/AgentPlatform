"""Bundled JSON templates must stay in sync with their schemas.
...
items), but at one point the sample template was not updated, so any agent
following it reproduced the same schema violation.  These tests guard every
``templates/*.json`` against its sibling ``*.schema.json``, so drift fails the
suite instead of the runtime schema gate.
"""
