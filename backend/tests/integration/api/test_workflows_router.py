...
class _AsyncSession:
    def __init__(self, *, resource=None, execute_error: Exception | None = None):
    ...
    async def execute(self, _stmt):
        ...
        return _ScalarResult(self.resource)
...
class TestListWorkflows:
    ...
    def test_list_without_user_returns_only_public_workflows(self):
        store = _make_workflow_store(
            list_result=(
                [
                    {"name": "private-wf", "description": "P", "version": "1.0", "steps_count": 1, "inputs": {}},
                    {"name": "public-wf", "description": "U", "version": "1.0", "steps_count": 1, "inputs": {}},
                ],
                2,
            )
        )
        ...
        metas = [
            {"visibility": "private", "owner_id": "owner", "department_id": None, "is_favorited": False},
            {"visibility": "public", "owner_id": "owner", "department_id": None, "is_favorited": True},
        ]
        with (
            _mock_get_workflow_store(store),
            patch("app.gateway.routers.workflows._workflow_store.load_meta", new_callable=AsyncMock, side_effect=metas),
        ):
            with TestClient(app) as client:
                resp = client.get("/api/workflows")
        assert resp.status_code == 200
        data = resp.json()
        assert [wf["name"] for wf in data["workflows"]] == ["public-wf"]
        assert data["workflows"][0]["is_favorited"] is True
...
class TestGetWorkflow:
    ...
    def test_get_workflow_without_user_rejects_private(self):
        store = _make_workflow_store(load_result=VALID_YAML)
        app = _make_no_user_app()
        meta = {"visibility": "private", "owner_id": "owner", "department_id": None, "version": 1}
        with _mock_get_workflow_store(store), _mock_workflow_meta(meta):
            with TestClient(app) as client:
                resp = client.get(f"/api/workflows/{WORKFLOW_NAME}")
        assert resp.status_code == 404
...
class TestExportWorkflow:
    def test_export_success(self):
        ...
        data = resp.json()
        assert data["name"] == WORKFLOW_NAME
        assert data["yaml_content"] == VALID_YAML
        assert data["visibility"] == "private"

    def test_export_without_user_rejects_private(self):
        store = _make_workflow_store(load_result=VALID_YAML)
        app = _make_no_user_app()
        meta = {"visibility": "private", "owner_id": "owner", "department_id": None, "version": 1}
        with _mock_get_workflow_store(store), _mock_workflow_meta(meta):
...
class TestImportWorkflow:
    def test_import_success_with_department_visibility(self):
        store = _make_workflow_store(load_result=None)
        app, user = _make_app()
        user.department_id = "dept-1"
        save_meta = AsyncMock(return_value=True)
        with (
            _mock_get_workflow_store(store),
            patch("app.gateway.routers.workflows._workflow_store.save_meta", save_meta),
        ):
            with TestClient(app) as client:
                resp = client.post(
                    "/api/workflows/import",
                    json={"yaml_content": VALID_YAML, "visibility": "department"},
                )
        assert resp.status_code == 201
        assert resp.json()["owner_id"] == "user-1"
        save_meta.assert_awaited_once_with(
            WORKFLOW_NAME,
            {"visibility": "department", "owner_id": "user-1", "department_id": "dept-1"},
        )
...
class TestToggleWorkflowFavorite:
    def test_toggles_existing_resource(self):
        app, _ = _make_app()
        resource = SimpleNamespace(is_favorited=False)
        session = _AsyncSession(resource=resource)

        with patch("app.gateway.routers.workflows.get_session_factory", return_value=_session_factory(session)):
            with TestClient(app) as client:
    ...
    def test_returns_500_when_database_unavailable(self):
        ...
        with patch("app.gateway.routers.workflows.get_session_factory", return_value=None):
            with TestClient(app) as client:
                resp = client.post(f"/api/workflows/{WORKFLOW_NAME}/favorite")
