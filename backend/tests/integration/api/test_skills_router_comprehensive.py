...
# ===========================================================================
# 14. GET /api/skills/custom/{skill_name}/export
# ===========================================================================


class TestExportCustomSkill:
    def test_export_custom_skill_success(self):
        storage = _make_mock_storage()
        storage.custom_skill_exists.return_value = True
        storage.read_custom_skill.return_value = "# Exported skill"
        meta = {"visibility": "private", "owner_id": "test-user", "department_id": None}
        ...
        with (
            ...
            app = _make_app()
            client = TestClient(app)
            resp = client.get("/api/skills/custom/my-skill/export")

        assert resp.status_code == 200
        assert resp.json() == {"name": "my-skill", "content": "# Exported skill", "meta": meta}

    def test_export_custom_skill_without_user_rejects_private(self):
        storage = _make_mock_storage()
        storage.custom_skill_exists.return_value = True

        with (
            patch("app.gateway.routers.skills.get_or_new_skill_storage", return_value=storage),
            patch("app.gateway.routers.skills._load_skill_meta", new_callable=AsyncMock, return_value={"visibility": "private", "owner_id": "owner"}),
        ):
            app = _make_no_user_app()
            client = TestClient(app)
            resp = client.get("/api/skills/custom/my-skill/export")

        assert resp.status_code == 404

    def test_export_custom_skill_not_found(self):
        storage = _make_mock_storage()
        storage.custom_skill_exists.return_value = False
        ...
        with patch("app.gateway.routers.skills.get_or_new_skill_storage", return_value=storage):
            app = _make_app()
            client = TestClient(app)
            resp = client.get("/api/skills/custom/missing/export")

        assert resp.status_code == 404

    def test_export_custom_skill_generic_exception(self):
        storage = _make_mock_storage()
        storage.custom_skill_exists.side_effect = RuntimeError("boom")
...
# ===========================================================================
# 15. POST /api/skills/custom/import
# ===========================================================================


class TestImportCustomSkill:
    def test_import_custom_skill_success_without_loaded_skill_fallback_response(self):
        user = _make_user(user_id="user-1", dept_id="dept-1")
        storage = _make_mock_storage([])
        storage.custom_skill_exists.return_value = False
        scan = _make_scan_result("allow", "ok")
        ...
        with (
            patch("app.gateway.routers.skills.get_or_new_skill_storage", return_value=storage),
            patch("app.gateway.routers.skills.scan_skill_content", new_callable=AsyncMock, return_value=scan),
            patch("app.gateway.routers.skills._save_skill_meta", new_callable=AsyncMock) as save_meta,
            patch("app.gateway.routers.skills.refresh_skills_system_prompt_cache_async", new_callable=AsyncMock),
        ):
            app = _make_app(user)
            client = TestClient(app)
            resp = client.post(
                ...
                json={"name": "my-skill", "content": "# Imported skill\nbody", "visibility": "department"},
            )

        assert resp.status_code == 201
        assert resp.json()["name"] == "my-skill"
        storage.write_custom_skill.assert_called_once()
        storage.append_history.assert_called_once()
        save_meta.assert_awaited_once_with(
            "my-skill",
            SimpleNamespace(),
            {"owner_id": "user-1", "department_id": "dept-1", "visibility": "department"},
        )

    def test_import_custom_skill_conflict(self):
        storage = _make_mock_storage()
        storage.custom_skill_exists.return_value = True
...
class TestDeprecatedSkillApplicationRoutes:
    ...
    def test_import_custom_skill_scanner_blocked(self):
        storage = _make_mock_storage()
        storage.custom_skill_exists.return_value = False
        scan = _make_scan_result("block", "blocked")
        ...
        with (
            ...
            resp = client.post(
                "/api/skills/custom/import",
                json={"name": "my-skill", "content": "# Bad skill"},
            )

        assert resp.status_code == 400
        assert "blocked" in resp.json()["detail"]

    def test_import_custom_skill_validation_error(self):
        storage = _make_mock_storage()
        storage.custom_skill_exists.return_value = False
        storage.validate_skill_markdown_content.side_effect = ValueError("invalid content")
