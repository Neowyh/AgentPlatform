...
from ideer.agents.memory.prompt import format_conversation_for_update
...
def test_apply_updates_skips_existing_duplicate_and_preserves_removals() -> None:
    ...
    with patch(
        "ideer.agents.memory.updater.get_memory_config",
...
def test_prepare_update_prompt_preserves_non_ascii_memory_text() -> None:
    ...
    with (
        ...
        patch("ideer.agents.memory.updater.get_memory_data", return_value=current_memory),
    ):
        msg = MagicMock()
        msg.type = "human"
        msg.content = "你好"
...
def test_clear_memory_data_resets_all_sections() -> None:
    with patch("ideer.agents.memory.updater._save_memory_to_file", return_value=True):
...
def test_delete_memory_fact_raises_for_unknown_id() -> None:
    with patch("ideer.agents.memory.updater.get_memory_data", return_value=_make_memory()):
        try:
...
def test_update_memory_fact_rejects_invalid_confidence() -> None:
    ...
    for confidence in (-0.1, 1.1, float("nan"), float("inf"), float("-inf")):
        with patch(
            "ideer.agents.memory.updater.get_memory_data",
...
class TestUpdateMemoryStructuredResponse:
    ...
    def test_sync_update_memory_returns_false_when_executor_down(self):
        ...
        with (
            patch(
                "ideer.agents.memory.updater._SYNC_MEMORY_UPDATER_EXECUTOR.submit",
...
class TestFinalizeCacheIsolation:
    ...
    def test_deepcopy_prevents_cache_corruption_on_save_failure(self):
        ...
        with (
            ...
            patch("ideer.agents.memory.updater.get_memory_data", return_value=original_memory),
            patch("ideer.agents.memory.updater.get_memory_storage", return_value=MagicMock(save=save_mock)),
        ):
            msg = MagicMock()
            msg.type = "human"
            msg.content = "hello"
...
class TestUserIdForwarding:
    ...
    def test_sync_update_forwards_user_id_to_load_and_save(self):
        ...
        with (
            patch.object(updater, "_get_model", return_value=model),
            patch("ideer.agents.memory.updater.get_memory_config", return_value=_memory_config(enabled=True)),
            patch("ideer.agents.memory.updater.get_memory_data", return_value=_make_memory()) as mock_load,
            patch("ideer.agents.memory.updater.get_memory_storage", return_value=mock_storage),
        ):
            msg = MagicMock()
            msg.type = "human"
            msg.content = "Hello"
