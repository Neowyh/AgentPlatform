...
Used by ``frontend/playwright.record.config.ts``. Env:
  ...
  IDEER_RECORD_OUT                  - JSONL path to append captured turns to
...
def _install_capture(out_path: Path) -> None:
    ...
    import ideer.models.factory as factory_mod
...
def main() -> int:
    ...
    record_out = os.environ.get("IDEER_RECORD_OUT")
    if not record_out:
        print("ERROR: set IDEER_RECORD_OUT to the JSONL path to append captured turns to", file=sys.stderr)
        return 2
    ...
    # IDEER_HOME can't leak in and shift prompt-affecting paths/skills.
    os.environ["IDEER_HOME"] = str(home)
    os.environ["IDEER_CONFIG_PATH"] = str(cfg)
    os.environ["IDEER_EXTENSIONS_CONFIG_PATH"] = str(prepare_hermetic_extras(home))
