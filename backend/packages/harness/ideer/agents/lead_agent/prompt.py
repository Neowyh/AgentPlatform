...
from ideer.skills.catalog import get_skill_catalog
...
def _load_enabled_skills_sync() -> list[Skill]:
    return get_skill_catalog(source=get_or_new_skill_storage()).list_skills(enabled_only=True)


def _start_enabled_skills_refresh_thread() -> None:
    threading.Thread(
        target=_refresh_enabled_skills_cache_worker,
        name="ideer-enabled-skills-loader",
        daemon=True,
    ).start()
...
def get_enabled_skills_for_config(app_config: AppConfig | None = None) -> list[Skill]:
    ...
    skills = get_skill_catalog(
        source=get_or_new_skill_storage(app_config=app_config),
...
iDeer is an open-source AI agent framework that gained significant traction in early 2026
[citation:GitHub Repository](https://github.com/bytedance/ideer). The project focuses on
providing a production-ready agent system with sandbox execution and memory management
[citation:iDeer Documentation](https://ideer.dev/docs).
...
### Primary Sources
- [GitHub Repository](https://github.com/bytedance/ideer) - Official source code and documentation
- [iDeer Documentation](https://ideer.dev/docs) - Technical specifications
...
- ✅ RIGHT in Sources: `[GitHub Repository](https://github.com/bytedance/ideer) - 官方源代码和文档`
...
def _get_memory_context(agent_name: str | None = None, *, app_config: AppConfig | None = None) -> str:
    ...
    """
    try:
        from ideer.agents.memory import format_memory_for_injection, get_memory_data
...
def _build_acp_section(*, app_config: AppConfig | None = None) -> str:
    ...
    if app_config is None:
        try:
            from ideer.config.acp_config import get_acp_agents
    ...
    *,
    ...
    if requested_skill_name:
        requested_skill_section = (
            ...
            f"When this task is relevant to that skill, prefer it and load its SKILL.md before using it.\n"
            f"Use `read_file` on the skill's main file located at the path provided in the <available_skills> section when you use it.\n"
            f"</requested_skill>"
        )
