"""Pure-argument factory for iDeer agents.

``create_ideer_agent`` accepts plain Python arguments — no YAML files, no
...
from ideer.agents.middlewares.tool_error_handling_middleware import ToolErrorHandlingMiddleware
from ideer.agents.thread_state import ThreadState
from ideer.tools.builtins import ask_clarification_tool
...
def create_ideer_agent(
    ...
    *,
    ...
    """Create a iDeer agent from plain Python arguments.
...
def _assemble_from_features(
    ...
    *,
    ...
    if plan_mode:
        from ideer.agents.middlewares.todo_middleware import TodoMiddleware
    ...
    if feat.vision is not False:
        ...
        if feat.sandbox is not False:
            from ideer.tools.builtins import view_image_tool
    ...
    if feat.subagent is not False:
        if isinstance(feat.subagent, AgentMiddleware):
            ...
            from ideer.agents.middlewares.subagent_limit_middleware import SubagentLimitMiddleware

            chain.append(SubagentLimitMiddleware())
        from ideer.tools.builtins import task_tool
    ...
    if feat.loop_detection is not False:
        if isinstance(feat.loop_detection, AgentMiddleware):
            ...
            from ideer.agents.middlewares.loop_detection_middleware import LoopDetectionMiddleware
