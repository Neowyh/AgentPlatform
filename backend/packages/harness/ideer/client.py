"""IDeerClient — Embedded Python client for iDeer agent system.

Provides direct programmatic access to iDeer's agent capabilities
...
from ideer.config.app_config import get_app_config, reload_app_config
...
from ideer.tracing import build_tracing_callbacks, inject_langfuse_metadata
...
class IDeerClient:
    """Embedded Python client for iDeer agent system.
        ...
        *,
        ...
        Args:
            ...
            environment: Deployment environment label that ends up in
                ...
                ``IDEER_ENV`` or ``ENVIRONMENT`` env vars. Pass an
    ...
    def _ensure_agent(self, config: RunnableConfig):
        ...
        if checkpointer is None:
            from ideer.runtime.checkpointer import get_checkpointer
    ...
    def _ai_tool_calls_event(msg_id: str | None, tool_calls, additional_kwargs: dict[str, Any] | None = None) -> "StreamEvent":
        ...
        data: dict[str, Any] = {
            ...
            "content": "",
            "id": msg_id,
            "tool_calls": IDeerClient._serialize_tool_calls(tool_calls),
        }
        if additional_kwargs:
            data["additional_kwargs"] = additional_kwargs
        return StreamEvent(type="messages-tuple", data=data)
    ...
    def _tool_message_event(msg: ToolMessage) -> "StreamEvent":
        ...
        return StreamEvent(
            ...
            data={
                "type": "tool",
                "content": IDeerClient._extract_text(msg.content),
                "name": msg.name,
    ...
    def _serialize_message(msg) -> dict:
        ...
        if isinstance(msg, AIMessage):
            ...
            if msg.tool_calls:
                d["tool_calls"] = IDeerClient._serialize_tool_calls(msg.tool_calls)
            if getattr(msg, "usage_metadata", None):
                d["usage_metadata"] = msg.usage_metadata
            if additional_kwargs := IDeerClient._serialize_additional_kwargs(msg):
    ...
    def list_threads(self, limit: int = 10) -> dict:
        ...
        checkpointer = self._checkpointer
        if checkpointer is None:
            from ideer.runtime.checkpointer.provider import get_checkpointer
    ...
    def stream(
        ...
        * ``StreamBridge`` is an asyncio-queue decoupling producers from
        ...
        So ``IDeerClient.stream()`` is a parallel, sync, in-process
        ...
        inject_langfuse_metadata(
            ...
            environment=self._environment or os.environ.get("IDEER_ENV") or os.environ.get("ENVIRONMENT"),
        )
    ...
    def get_memory(self) -> dict:
        ...
        """
        from ideer.agents.memory.updater import get_memory_data
    ...
    def import_memory(self, memory_data: dict) -> dict:
        ...
        from ideer.agents.memory.updater import import_memory_data
    ...
    def update_mcp_config(self, mcp_servers: dict[str, dict]) -> dict:
        ...
        if config_path is None:
            raise FileNotFoundError("Cannot locate extensions_config.json. Set IDEER_EXTENSIONS_CONFIG_PATH or ensure it exists in the project root.")
    ...
    def reload_memory(self) -> dict:
        ...
        """
        from ideer.agents.memory.updater import reload_memory_data

        return reload_memory_data(user_id=get_effective_user_id())

    def clear_memory(self) -> dict:
        """Clear all persisted memory data."""
        from ideer.agents.memory.updater import clear_memory_data
    ...
    def create_memory_fact(self, content: str, category: str = "context", confidence: float = 0.5) -> dict:
        ...
        from ideer.agents.memory.updater import create_memory_fact

        return create_memory_fact(content=content, category=category, confidence=confidence)

    def delete_memory_fact(self, fact_id: str) -> dict:
        """Delete a single fact from memory by fact id."""
        from ideer.agents.memory.updater import delete_memory_fact
    ...
    def update_memory_fact(
        ...
        from ideer.agents.memory.updater import update_memory_fact
