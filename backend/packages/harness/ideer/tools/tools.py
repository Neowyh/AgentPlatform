...
def _is_host_bash_tool(tool: object) -> bool:
    ...
    if group == "bash":
        return True
    if use == "ideer.sandbox.tools:bash_tool":
        return True
    return False
...
def get_available_tools(
    ...
    *,
    ...
    `initialize_mcp_tools()` from ideer.mcp module.
            ...
            if extensions_config.get_enabled_mcp_servers():
                ...
                if mcp_tools:
                    ...
                    if config.tool_search.enabled:
                        from ideer.tools.builtins.tool_search import DeferredToolRegistry, set_deferred_registry
                        from ideer.tools.builtins.tool_search import tool_search as tool_search_tool
                        ...
                        if existing_registry is None:
                            ...
                            priority_names = {tool.name for tool in loaded_tools + builtin_tools}
                            for t in mcp_tools:
                                if t.name in priority_names:
                                    continue
                                registry.register(t)
    ...
    try:
        from ideer.tools.builtins.invoke_acp_agent_tool import build_invoke_acp_agent_tool
    ...
    priority_names = {tool.name for tool in loaded_tools + builtin_tools}
    deferred_names = set(deferred_names) - priority_names
