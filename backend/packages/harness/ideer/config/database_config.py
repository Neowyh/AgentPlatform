...
Controls BOTH the LangGraph checkpointer and the iDeer application
...
({sqlite_dir}/ideer.db) with WAL journal mode enabled on every
...
class DatabaseConfig(BaseModel):
    ...
    sqlite_dir: str = Field(
        default=".ideer/data",
        description=("Directory for the SQLite database file. Both checkpointer and application data share {sqlite_dir}/ideer.db."),
    )
    postgres_url: str = Field(
        default="",
        description=(
            "PostgreSQL connection URL, shared by checkpointer and app. Use $DATABASE_URL in config.yaml to reference .env. Example: postgresql://user:pass@host:5432/ideer (the +asyncpg driver suffix is added automatically where needed)."
        ),
    )
    echo_sql: bool = Field(
        default=False,
    ...
    def sqlite_path(self) -> str:
        ...
        return os.path.join(self._resolved_sqlite_dir, "ideer.db")
