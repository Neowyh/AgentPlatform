...
from ideer.config.acp_config import ACPAgentConfig, load_acp_config_from_dict
from ideer.config.agents_api_config import AgentsApiConfig, load_agents_api_config_from_dict
from ideer.config.checkpointer_config import CheckpointerConfig, load_checkpointer_config_from_dict
from ideer.config.database_config import DatabaseConfig
from ideer.config.extensions_config import ExtensionsConfig
from ideer.config.guardrails_config import GuardrailsConfig, load_guardrails_config_from_dict
from ideer.config.loop_detection_config import LoopDetectionConfig
from ideer.config.memory_config import MemoryConfig, load_memory_config_from_dict
...
from ideer.config.stream_bridge_config import StreamBridgeConfig, load_stream_bridge_config_from_dict
from ideer.config.subagents_config import SubagentsAppConfig, load_subagents_config_from_dict
from ideer.config.summarization_config import SummarizationConfig, load_summarization_config_from_dict
from ideer.config.title_config import TitleConfig, load_title_config_from_dict
...
def apply_logging_level(name: str | None) -> None:
    """Resolve *name* to a logging level and apply it to the ``ideer``/``app`` logger hierarchies.

    Only the ``ideer`` and ``app`` logger levels are changed so that
...
class AppConfig(BaseModel):
    """Config for the iDeer application"""

    log_level: str = Field(default="info", description="Logging level for ideer and app modules (debug/info/warning/error); third-party libraries are not affected")
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `IDEER_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("IDEER_CONFIG_PATH"):
            path = Path(os.getenv("IDEER_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `IDEER_CONFIG_PATH` not found at {path}")
            return path
        else:
...
_current_app_config: ContextVar[AppConfig | None] = ContextVar("ideer_current_app_config", default=None)
_current_app_config_stack: ContextVar[tuple[AppConfig | None, ...]] = ContextVar("ideer_current_app_config_stack", default=())
