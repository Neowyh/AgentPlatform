...
and the embedded :class:`~ideer.client.IDeerClient`.
...
Usage::
    ...
    from ideer.runtime.store.provider import get_store, store_context
...
from ideer.runtime.store._sqlite_utils import ensure_sqlite_parent_dir, resolve_sqlite_conn_str
...
POSTGRES_STORE_INSTALL = "langgraph-checkpoint-postgres is required for the PostgreSQL store. Install the package extra with: pip install 'ideer-harness[postgres]' (or use: uv sync --all-packages --extra postgres when developing locally)"
...
def _sync_store_cm(config) -> Iterator[BaseStore]:
    ...
    :class:`~ideer.config.checkpointer_config.CheckpointerConfig` instance —
...
def get_store() -> BaseStore:
    ...
    from ideer.config.app_config import _app_config
