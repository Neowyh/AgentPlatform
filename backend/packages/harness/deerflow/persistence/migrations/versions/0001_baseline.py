...
PATCH (2026-09-08, unified migration chain / PATCH-006 closure)
===============================================================

Every ``create_table`` in ``upgrade()`` is now guarded with an inspector
check (mirroring ``Base.metadata.create_all``'s ``checkfirst`` semantics).
Reason: the unified chain (merge revision
``20260908_unify_migration_chains``) can run this revision against databases
that already hold the five core tables (``runs``, ``threads_meta``,
``run_events``, ``feedback``, ``users``) created by the control-plane chain
(``c4d5e6f7a8b9``) -- e.g. pre-convergence deployments recorded at the
control-plane head, where the upgrade pulls in the runtime branch through the
merge. Guarding changes nothing for existing databases: no bootstrap branch
has ever executed this ``upgrade()`` on a real database (see the three-branch
table above), so the guarded DDL has never been applied anywhere; the guard
only makes the from-scratch and merge paths tolerant of the control-plane
chain's tables. Skipped tables keep the index set created alongside them by
``c4d5e6f7a8b9``/``35830514e3ee`` (same index names); the one known
divergence -- ``idx_users_oauth_identity`` partial (this file) vs.
non-partial (``c4d5e6f7a8b9``) on SQLite -- is pre-existing behavior and is
not reconciled here. Recorded in
...
def upgrade() -> None:
    """Upgrade schema (create_table guarded -- see module docstring)."""
    existing = set(sa.inspect(op.get_bind()).get_table_names())
    # ### commands auto generated by Alembic - please adjust! ###
    if "channel_connections" not in existing:
        op.create_table(
            "channel_connections",
    ...
    if "channel_oauth_states" not in existing:
        op.create_table(
    ...
    if "channel_conversations" not in existing:
        op.create_table(
    ...
    if "channel_credentials" not in existing:
        op.create_table(
            "channel_credentials",
            sa.Column("connection_id", sa.String(length=64), nullable=False),
