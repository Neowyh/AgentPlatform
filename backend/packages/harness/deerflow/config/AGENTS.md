...
- `memory` - Host-shared memory contract (`enabled`, `mode`, `injection_enabled`, `shutdown_flush_timeout_seconds`, `manager_class`, and `backend_config`). Backend-private DeerMem settings, including the directory `storage_path`, belong under `memory.backend_config`; legacy top-level fields are migrated on load but must not be added to new profiles.
