...
async def delete_thread_data(thread_id: str, request: Request) -> ThreadDeleteResponse:
    ...
    (sqlite or memory).
    """
    # Reject malformed route IDs before resolving optional runtime services;
    # validation errors must remain 422 even in a minimally wired test or
    # degraded application where the run manager is unavailable.
    try:
...
async def get_thread(thread_id: ThreadId, request: Request) -> ThreadResponse:
    ...
    try:
        ...
        checkpoint_id = (snapshot.config or {}).get("configurable", {}).get("checkpoint_id") if snapshot is not None else None
    ...
    if snapshot is None:
        # Thread created but no checkpoint written yet — the metadata row is
        # the only source of identity, and there is no state to serialize.
        return ThreadResponse(
            thread_id=thread_id,
            status=record.get("status", "idle"),
            created_at=coerce_iso(record.get("created_at", "")),
    ...
    serialized_values = serialize_channel_values_for_api(snapshot.values)
    serialized_messages = serialized_values.get("messages")
    if isinstance(serialized_messages, list):
        serialized_values["messages"] = await stamp_messages_with_seq(getattr(request.app.state, "run_event_store", None), thread_id, serialized_messages)
        ...
        for snapshot in snapshots:
            ...
            if is_latest_checkpoint:
                ...
                if messages:
                    ...
                    serialized_msgs = await stamp_messages_with_seq(getattr(request.app.state, "run_event_store", None), thread_id, serialized_msgs)
                    try:
