...
from .pairing import PairingError, consume_pairing_code, hash_secret, new_pairing_code, validate_device_transition
...
class DeviceControlService:
    ...
    async def heartbeat(self, device_id: str, token: str, *, capabilities: list[str] | None = None) -> DeviceModel:
        ...
        if device.status in {DeviceStatus.REVOKED, DeviceStatus.BLOCKED}:
            raise DeviceControlError("DEVICE_REVOKED" if device.status == DeviceStatus.REVOKED else "DEVICE_BLOCKED", "device cannot connect", 403)
    ...
    async def revoke(self, device_id: str, *, actor_id: str, is_admin: bool) -> DeviceModel:
        ...
        if device.owner_id != actor_id and not is_admin:
            raise DeviceControlError("DEVICE_FORBIDDEN", "only the owner or an administrator may revoke this device", 403)
