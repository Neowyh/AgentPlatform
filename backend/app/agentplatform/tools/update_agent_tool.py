"""update_agent tool — let a custom agent persist updates to the canonical catalog.

Restored from the deleted ``ideer.tools.builtins.update_agent_tool``. The
update goes through the canonical resource catalog (draft + publish) so the
catalog row, filesystem draft, dependencies, and published version stay
consistent — mirroring the /resources agent-draft and publish endpoints.
"""
...
def update_agent(
    ...
    # Reject an unknown ``model`` *before* touching the catalog. Otherwise
