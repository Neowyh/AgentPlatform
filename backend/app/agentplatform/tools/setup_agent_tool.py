"""Enterprise ``setup_agent`` tool.

Restored from the deleted ``ideer.tools.builtins.setup_agent_tool``: agents
created by the model become canonical resource catalog entries owned by the
authenticated user (mirroring the /resources agent-draft + publish endpoints),
so Resource Governance keeps applying to chat-created agents.
"""
