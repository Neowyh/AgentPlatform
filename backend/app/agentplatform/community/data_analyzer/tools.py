"""Data Analyzer Tool - Analyze structured data files and generate statistical insights.
...
def _read_file(file_path: str):
    ...
    # Post-load row limit safety net (nrows may not be respected by all formats)
    if len(df) > _MAX_ROWS:
        logger.warning("DataFrame has %d rows, truncating to %d", len(df), _MAX_ROWS)
...
def _analyze_summary(df) -> dict:
    ...
    try:
        ...
        # to_markdown requires tabulate; fall back to string
...
@tool("data_analyzer", parse_docstring=True)
def data_analyzer_tool(file_path: str, analysis_type: str = "summary") -> str:
