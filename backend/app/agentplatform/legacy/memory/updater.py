...
class MemoryUpdater:
    ...
    def _get_model(self):
        ...
        model_name = self._model_name or legacy_memory_setting(config, "model_name", None)
    ...
    def _apply_updates(
        ...
        for fact in new_facts:
            ...
            if confidence >= float(legacy_memory_setting(config, "fact_confidence_threshold", LEGACY_FACT_CONFIDENCE_THRESHOLD)):
        ...
        max_facts = int(legacy_memory_setting(config, "max_facts", LEGACY_MAX_FACTS))
        if len(current_memory["facts"]) > max_facts:
