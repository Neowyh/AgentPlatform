"""Enterprise SKILL.md frontmatter validation.

Migrated from the deleted ``ideer.skills.validation`` module. The upstream
:mod:`deerflow.skills.validation` accepts only the upstream frontmatter key
set, while enterprise Skills additionally carry i18n / governance keys
(``description_zh``, ``visibility``, ``category`` ...). Both the bundled
skills under ``resources/skills/`` and the admin Skill draft editor rely on
the enterprise key set, so the gateway must validate against it.
"""
...
from app.agentplatform.resources.skill_parser import parse_allowed_tools
from app.agentplatform.resources.skill_types import SKILL_MD_FILE

# Allowed properties in SKILL.md frontmatter (enterprise superset)
