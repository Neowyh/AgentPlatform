...
from .record import CharTypeface, EmbellType, MtAST, MtChar, MtColorDef, MtColorDefIndex, MtEmbellRd, MtEqnPrefs, MtfontDef, MtfontStyleDef, MtLine, MtMatrix, MtPile, MtSize, MtTmpl, OptionType, RecordType, SelectorType
...
class MTEF:
    ...
    def readRecord(self):
        ...
        while True:
            ...
            if record == RecordType.END:
                ...
                # 匹配矩阵数据下面的2个nil (保持null=False以匹配END栈)
    ...
    def readLine(self, line):
        ...
        if OptionType.mtefOPT_LP_RULER == OptionType.mtefOPT_LP_RULER & options:
            ...
            for i in range(nStops):
                ...
                Helper.bytes2int(self.reader.read(2))  # uint16
    ...
    def readEqnPrefs(self, eqnPrefs):
        Helper.bytes2int(self.reader.read(1))
            ...
            if node.tag == RecordType.EMBELL:
                if len(stack):
                    ...
                    embellTypesToSwap = {
                        ...
                        EmbellType.embHAT,
                        EmbellType.embNOT,
                        EmbellType.embRARROW,
                        EmbellType.embMBAR,
                        EmbellType.embOBAR,
                        EmbellType.embLARROW,
                    }

                    if embellType in embellTypesToSwap:
                        # 检查栈顶是否是EMBELL（嵌套EMBELL，如\dot{\vec{V}}）
                        if parent.tag == RecordType.EMBELL:
                            # 弹出前一个EMBELL，反转嵌套顺序
                            # 后出现的EMBELL应该包裹先出现的EMBELL
                            prevEmbell = stack.pop()
                            realParent = stack[-1]  # 真正的容器（LINE等）
                            if not realParent.children:
                                realParent.children = []
                            # 从真正容器中移除前一个EMBELL
                            if realParent.children and realParent.children[-1] is prevEmbell:
                                realParent.children.pop()
                            # 前一个EMBELL成为新EMBELL的孩子
                            node.children = [prevEmbell]
                            # 新EMBELL添加到真正容器
                            realParent.children.append(node)
                        else:
                            # 非嵌套：将EMBELL前面的字符移入EMBELL内部
                            if not parent.children:
                                parent.children = []
                            if len(parent.children) >= 1:
                                lastChild = parent.children[-1]
                                if lastChild.tag == RecordType.CHAR:
                                    parent.children.pop()
                                    node.children = [lastChild]
                            parent.children.append(node)
                    else:
                        # 不需要交换的EMBELL（如prime标记），直接追加
                        if not parent.children:
                            parent.children = []
                        parent.children.append(node)
    ...
    def makeLatex(self, ast):
        ...
        if ast.tag == RecordType.ROOT:
            ...
            hexCode = f"{mtcode:04x}"
            # hexKey := fmt.Sprintf("char/0x%s%s", hexCode, hexExtend)
            hexKey = f"char/0x{hexCode}{hexExtend}"
            ...
            if not sChar:
                # 尝试查找mathmode版本（希腊字母等通常只存在mathmode映射中）
                sChar = Chars.get(f"char/0x{hexCode}/mathmode")
            if sChar:
                char = sChar
            else:
            ...
            if tmpl.selector == SelectorType.tmANGLE:
                ...
                if mainSlot != "":
                    mainStr = f"{{ {mainSlot} }}"
                if leftSlot != "":
                    leftStr = f"\\left {leftSlot}"
                if rightSlot != "":
                    rightStr = f"\\right {rightSlot}"

                buf += f"{leftStr} {mainStr} {rightStr}"
                ...
                for astData in ast.children:
                    child_latex, _ = self.makeLatex(astData)
                    if idx == 0:
                        mainSlot = child_latex
                    elif idx == 1:
                        leftSlot = child_latex
                    else:
                        rightSlot = child_latex
                    idx += 1
                ...
                # 组装公式
                buf += f"\\left {leftSlot} \\begin{{array}}{{l}} {mainSlot} \\end{{array}} \\right{rightSlot}"
                ...
                leftSlot, _ = self.makeLatex(leftAST)
                rightSlot, _ = self.makeLatex(rightAST)
                buf += f"\\left{leftSlot} {mainSlot} \\right{rightSlot}"
                return buf, None
                ...
                # 组成整体公式
                tmplStr = f"{leftStr} {mainStr} {rightStr}"
                buf += tmplStr
                ...
                bottomStr = ""
                if topSlot != "":
                    topStr = f"{{\\mathrm{{ {topSlot} }}}}"
                if bottomSlot != "":
                    bottomStr = f"[\\mathrm{{ {bottomSlot} }}]"
                ...
                # 组成整体公式
                tmplStr = f"{latexFmt} {bottomStr} {topStr}"
                buf += tmplStr
                ...
                if mainSlot != "":
                    mainStr = f"{{ {mainSlot} }}"
                if lowerSlot != "":
                    lowerStr = f"\\limits_{{ {lowerSlot} }}"
                if upperSlot != "":
                    upperStr = f"^ {upperSlot}"

                # 组成整体公式
                tmplStr = f"{operatorSlot} {lowerStr} {upperStr} {mainStr}"
                buf += tmplStr
                ...
                # 组成整体公式
                tmplStr = f"{mainStr} {lowerStr} {upperStr}"
                buf += tmplStr
            ...
            matrixRows = int(ast.value.rows)
            numCells = matrixRows * matrixCol
            buf += " \\begin{array} {" + "l" * matrixCol + "} "

            # MATRIX的直接孩子结构：child[0]是null_LINE_1，之后是实际单元格LINE
            # END会在处理body内容前弹出null_2和null_1，因此单元格是MATRIX的直接孩子
            cells = []
            if len(ast.children) > 1:
                cells = [c for c in ast.children[1:] if c.tag == RecordType.LINE]

            for i in range(min(numCells, len(cells))):
                _latex, _ = self.makeLatex(cells[i])
                buf += _latex
                if i < numCells - 1:  # not the last cell
                    if (i + 1) % matrixCol == 0:
                        buf += " \\\\ "
                    else:
                        buf += " & "
            ...
            if embellType == EmbellType.emb1DOT:
                embellStr = " \\dot "
            elif embellType == EmbellType.emb2DOT:
                embellStr = " \\ddot "
                ...
                embellStr = " \\tilde "
            elif embellType == EmbellType.embNOT:
                embellStr = " \\not "
            elif embellType == EmbellType.embRARROW:
                embellStr = " \\vec "
            elif embellType == EmbellType.embLARROW:
                embellStr = " \\overleftarrow "
            elif embellType == EmbellType.embMBAR:
                embellStr = " \\overline "
            elif embellType == EmbellType.embOBAR:
                embellStr = " \\bar "
            elif embellType == EmbellType.embFROWN:
                embellStr = " \\frown "
            elif embellType == EmbellType.embSMILE:
                embellStr = " \\smile "
            else:
                print("METF.makeLatex:not implement embell:", embellType)

            buf += embellStr
            # 处理EMBELL的孩子（嵌套的EMBELL、下标等），防止内容丢失
            # 用大括号包裹孩子，确保LaTeX正确识别参数（如 \vec { V } 而非 \vec V）
            if ast.children:
                buf += "{ "
                for _ast in ast.children:
                    _latex, _ = self.makeLatex(_ast)
