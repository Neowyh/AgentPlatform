...
app.gateway.routers.uploads ──┤── calls ──→  ideer.skills.installer
ideer.client             ──┘              ideer.uploads.manager
```

- Shared modules live in the harness layer (`ideer.*`), pure business logic, no FastAPI dependency
...
| Alternative | Why Not |
|-------------|---------|
| Keep logic in Gateway, Client calls Gateway via HTTP | Adds network dependency to embedded Client; defeats the purpose of `iDeerClient` as an in-process API |
...
**None.** All public APIs (Gateway HTTP endpoints, `iDeerClient` methods) retain their existing signatures and return formats. The `SkillAlreadyExistsError` is a subclass of `ValueError`, so existing `except ValueError` handlers still catch it.
