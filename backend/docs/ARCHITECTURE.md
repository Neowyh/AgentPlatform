...
This document provides a comprehensive overview of the iDeer backend architecture.
...
**Entry Point**: `packages/harness/ideer/agents/lead_agent/agent.py:make_lead_agent`
...
The web conversation delete flow first deletes Gateway-managed thread state through the LangGraph-compatible route, then the Gateway `threads.py` router removes iDeer-managed filesystem data via `Paths.delete_thread_dir()`.
...
2. Gateway receives file
   - Validates file
   - Stores in .ideer/threads/{thread_id}/user-data/uploads/
...
3. Gateway removes local iDeer-managed files
   - Deletes .ideer/threads/{thread_id}/ recursively
