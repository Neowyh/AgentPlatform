...
> status: archived; implementation is already in the current codebase
> source date: 2026
> canonical-path: `backend/docs/archive/2026/task-tool-improvements.md`
...
Located in `packages/harness/ideer/tools/builtins/task_tool.py`:
...
**Configuration** (`packages/harness/ideer/subagents/config.py`):
```python
@dataclass
