...
> source date: 2026
> canonical-path: `backend/docs/archive/2026/auth-test-docker-gap.md`

This file documents the only **un-executed** test cases from
the archived auth test plan after the full release validation pass.
...
|---|---|---|---|
| TC-DOCKER-01 | `ideer.db` volume persistence | Verify the `IDEER_HOME` bind mount survives container restart | needs `docker compose up` |
...
| TC-DOCKER-05 (credential surfacing) | `reset_admin` writes `.ideer/admin_initial_credentials.txt` with mode 0600 and logs only the path — the only Docker-unique step is whether the bind mount projects this path onto the host, which is a `docker compose` config check, not a runtime behavior change |
...
# Optional: pin IDEER_HOME to a stable host path
echo "IDEER_HOME=$HOME/ideer-data" >> .env
```
