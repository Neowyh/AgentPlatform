...
- `--bundle-root` 和 `DEER_FLOW_BUNDLE_ROOT` 用于显式指定 bundle root，适合不在部署目录内执行脚本的场景。
...
| `deer-flow-source-<version>.tar.gz` | 运行所需源码、compose、脚本、基础配置和归零排故智能体预置可用配置 |
...
- `prepare`、`up/start`、`restart` 默认会安装共享归零排故智能体；`DEER_FLOW_INSTALL_FAULT_ZEROING=0` 可临时跳过。
