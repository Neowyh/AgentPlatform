...
本方案用于在不能访问公网的企业内网中部署 DeerFlow。整体思路是：先在一台可以访问依赖源或企业镜像源的构建机上打包，再把离线包拷贝到内网服务器上运行。
...
2. 命令参数：`./deploy-intranet.sh --bundle-root /home/deploy/deer-flow up`。
3. 环境变量：`DEER_FLOW_BUNDLE_ROOT=/home/deploy/deer-flow ./deploy-intranet.sh up`。

原理说明：

- `deploy-intranet.sh` 会把部署根目录记作 bundle root。
- bundle root 下会生成 `source/`、`runtime/`、`env.intranet`。
- 容器内路径是固定的，例如 `/app/backend/config.yaml`；宿主机路径是可配置的，例如 `/home/deploy/deer-flow/runtime/config.yaml`。
...
```text
dist/intranet/deer-flow-<version>/
```
...
| 文件 | 用途 |
| --- | --- |
| `deer-flow-images-<version>.tar` | Docker 镜像包，内网服务器用 `docker load` 导入 |
| `deer-flow-source-<version>.tar.gz` | 运行所需源码、compose、脚本、模板文件 |
| `docker-compose.intranet.yaml` | 参考 compose 文件，不建议直接运行 |
| `env.intranet.example` | 环境变量模板，路径示例为 `/home/deploy/deer-flow` |
...
| 文件 | 谁读取 | 作用 |
| --- | --- | --- |
| `runtime/config.yaml` | Gateway | DeerFlow 主配置，模型、工具、沙箱等配置在这里 |
...
1. 解压 `deer-flow-source-<version>.tar.gz` 到 `source/`。
...
- `DEER_FLOW_INTERNAL_AUTH_TOKEN` 会从 `runtime/data/.internal-auth-token` 复用，保证多 worker 内部调用一致。
...
ls runtime/config.yaml runtime/.env runtime/frontend.env runtime/extensions_config.json
