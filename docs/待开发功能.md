...
**路径**: `backend/.ideer/users/{super_admin_id}/agents/fault-zeroing/`

写入 `config.yaml`（内容同 `docs/fault-zeroing-agent/agent/config.yaml`）：
```yaml
name: fault-zeroing
...
写入 `SOUL.md`（内容同 `docs/fault-zeroing-agent/agent/SOUL.md`，57 行）。

#### 4.2 注册 ResourceMetadata

```sql
INSERT INTO resource_metadata (id, resource_type, resource_id, owner_id, visibility, version, is_favorited, created_at, updated_at)
VALUES ('{uuid}', 'agent', 'fault-zeroing', '{super_admin_id}', 'private', 1, 0, datetime('now'), datetime('now'));
```

其中 `super_admin_id` = `d1775756-bd3e-4703-b115-4137b8de172b`。
...
| `skills/custom/fault-zeroing/SKILL.md` | 关联的自定义 Skill（未删除） |
