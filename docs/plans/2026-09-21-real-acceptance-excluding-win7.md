...
5. 先 0/1 文件试跑，验证附件可访问且测量可解释，再执行 0/1/5/10/20，每个条件至少 5 个有效样本。每次创建新 Thread/Run，固定资料组合、模型和 Agent 版本；冷/热 AIO、缓存、连接状态分别记录，不能混成一组。所有失败和无正文样本保留并计入尝试数，不靠反复重试隐去失败率。
