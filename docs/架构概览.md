...
DeerFlow 是一个基于 LangGraph 的 AI 超级智能体系统，采用全栈架构。后端提供“超级智能体”能力，包含沙箱执行、持久化记忆、子智能体委托以及可扩展工具集成，且所有能力都运行在线程级隔离环境中。
...
- `make dev`、Docker 开发环境和生产环境都会通过 Gateway 中的 `RunManager` + `run_agent()` + `StreamBridge`（`packages/harness/deerflow/runtime/`）运行智能体运行时。Nginx 将该运行时暴露在 `/api/langgraph/*`，并重写到 Gateway 原生的 `/api/*` 路由。
...
- **Harness**（`packages/harness/deerflow/`）：可发布的智能体框架包（`deerflow-harness`）。导入前缀为 `deerflow.*`。包含智能体编排、工具、沙箱、模型、MCP、技能、配置等构建和运行智能体所需的一切。
- **App**（`app/`）：不发布的应用代码。导入前缀为 `app.*`。包含 FastAPI Gateway API 和 IM 渠道集成（飞书、Slack、Telegram、钉钉）。

**依赖规则**：App 可以导入 deerflow，但 deerflow 绝不能导入 app。该边界由 `tests/test_harness_boundary.py` 强制执行，并在 CI 中运行。
...
Lead-agent 中间件在 `packages/harness/deerflow/agents/middlewares/tool_error_handling_middleware.py`（`build_lead_runtime_middlewares`）和 `packages/harness/deerflow/agents/lead_agent/agent.py`（`_build_middlewares`）中按严格追加顺序组装：

1. **ThreadDataMiddleware**：在用户隔离范围下创建线程级目录（`backend/.deer-flow/users/{user_id}/threads/{thread_id}/user-data/{workspace,uploads,outputs}`）；通过 `get_effective_user_id()` 解析 `user_id`（无认证模式回退到 `"default"`）；Web UI 删除线程时，现在会在 LangGraph 删除线程之后，由 Gateway 清理本地线程目录
...
| **Threads**（`/api/threads/{id}`）                      | `DELETE /`：在 LangGraph 删除线程后移除 DeerFlow 管理的本地线程数据；意外失败会在服务端记录日志，并返回通用 500 detail                                                                                                                                                                                                          |
...
**沙箱工具**（位于 `packages/harness/deerflow/sandbox/tools.py`）：
...
### 子智能体系统（`packages/harness/deerflow/subagents/`）
...
该系统将外部消息平台（飞书、Slack、Telegram、钉钉）通过 LangGraph Server 连接到 DeerFlow 智能体。
...
- 在 Docker Compose 中，IM 渠道运行在 `gateway` 容器内，因此 `localhost` 指回该容器。`langgraph_url` 使用 `http://gateway:8001/api`，`gateway_url` 使用 `http://gateway:8001`，或设置 `DEER_FLOW_CHANNELS_LANGGRAPH_URL` / `DEER_FLOW_CHANNELS_GATEWAY_URL`
...
- 每个智能体的用户级记忆存储在 `{base_dir}/users/{user_id}/agents/{agent_name}/memory.json`
- 自定义智能体定义（`SOUL.md` + `config.yaml`）也按用户存储在 `{base_dir}/users/{user_id}/agents/{agent_name}/`。旧的共享布局 `{base_dir}/agents/{agent_name}/` 仍作为未迁移安装的只读回退
...
- vLLM 推理模型应使用 `deerflow.models.vllm_provider:VllmChatModel`；对于 Qwen 风格 parser，优先使用 `when_thinking_enabled.extra_body.chat_template_kwargs.enable_thinking`，DeerFlow 也会规范化旧版 `thinking` 别名
...
两者都可通过 Gateway API 端点或 `DeerFlowClient` 方法在运行时修改。

### 嵌入式客户端（`packages/harness/deerflow/client.py`）

`DeerFlowClient` 提供不经过 HTTP 服务、在进程内直接访问全部 DeerFlow 能力的方式。所有返回类型都与 Gateway API 响应 schema 对齐，因此消费代码在 HTTP 和嵌入模式下可以保持一致。

**架构**：导入与 Gateway API 相同的 `deerflow` 模块。共享同一套配置文件和数据目录。没有 FastAPI 依赖。
...
- 完整设计见 [docs/STREAMING.md](backend/docs/STREAMING.md)：说明 Gateway 和 DeerFlowClient 为什么是并行路径、LangGraph 的 `stream_mode` 语义、按 id 去重的不变量以及回归测试策略
...
**与 Gateway 的关键区别**：Upload 接受本地 `Path` 对象而不是 HTTP `UploadFile`，在复制前拒绝目录路径，并且当文档转换必须在活动事件循环中运行时复用单个 worker。Artifact 返回 `(bytes, mime_type)` 而不是 HTTP Response。新的 Gateway-only 线程清理路由会在 LangGraph 删除线程后删除 `.deer-flow/threads/{thread_id}`；目前还没有对应的 `DeerFlowClient` 方法。`update_mcp_config()` 和 `update_skill()` 会自动使缓存的智能体失效。
...
- 如果某个模块在测试中导致循环导入问题，在 `tests/conftest.py` 中添加 `sys.modules` mock（参考 `deerflow.subagents.executor` 的现有示例）
