...
|---|---|---|---|---|---|
| Runtime / context | locked `0f7d8709` | Focused foundation slices green; 175 textual `ideer` imports remain after migrating safety, migration, suggestions and run serialization callers | DeerFlow runtime; AgentPlatform extension for enterprise context | Runtime Foundation focused evidence and 27/44 run-router regressions in `BASELINE_REPORT.md` | Migrate remaining control-plane callers and remove production `ideer` imports |
