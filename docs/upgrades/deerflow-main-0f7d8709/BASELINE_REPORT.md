...
| `cd frontend && pnpm test` / `bash scripts/run-test-lane.sh frontend-standard` | blocked | The local pnpm client immediately fails with `unable to open database file`; no frontend tests start. |
| `cd frontend && pnpm check` | not run | Frontend dependency/tooling setup is blocked by the same pnpm database error. |
