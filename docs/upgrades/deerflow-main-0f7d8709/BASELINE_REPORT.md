...
| MCP/Sandbox runtime configuration regressions | partial | MCP task runtime configuration: 9 passed; sandbox network policy: 12 passed; the sandbox authorization module has 13 synchronous policy tests passing, while two TestClient upload/artifact cases remain incomplete after the old route-config seam was removed. Gateway lifespan tests are still stale against removed OIDC/staging/retrieval symbols (12 collection-time failures). |
