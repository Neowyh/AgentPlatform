...
| GitNexus refresh | passed with bounded-flow warnings | Full rebuild completed in 220.4s at current commit `edcb74ef`: 91,329 nodes, 180,760 edges, 2,781 clusters and 1,013 flows. Analyzer reports bounded candidate/trace truncation for some high-branching flows; an empty impact result is not treated as proof of no callers. |
...
The branch still has 179 textual `ideer` imports under `backend/app`,
