...
| Auth/RBAC/Admin | `backend/tests/unit/gateway/*auth*`, admin helpers | `backend/tests/integration/api/*auth*`, `*admin*`, `test_audit_logs.py`, `test_user_deletion.py` | `backend/tests/contracts/test_rbac_matrix.py`, `test_rbac_permission_matrix_extended.py`, `test_permission_model_edge_cases.py`, `test_rbac_security.py` | `frontend/tests/e2e/auth/`, admin workflow specs | Role matrix, department-admin read access, user-deletion lifecycle, and permission-model files are the deletion gate for auth patch tests. |
