...
| PostgreSQL live schema/migration tests | unexecuted: isolated PostgreSQL 16 accepted connections, but `asyncpg`/`psycopg` installation was blocked by unavailable package network; the container was removed after the attempt |
...
- PostgreSQL migration verification remains `unexecuted`: an isolated PostgreSQL 16 container started and accepted connections, but the live tests could not import `asyncpg`/`psycopg`; the authorized `uv sync --all-packages --extra postgres` download timed out against PyPI and the fallback Ubuntu package download was also unreachable. The temporary container was removed. SQLite and migration unit coverage remain available locally.
