...
- Latest formal API setup reached KB creation, dataset binding, and document upload in a fresh SQLite Gateway, but the configured provider then returned `unavailable` during ingestion because no RAGFlow service/API key was available in this environment. The earlier standalone provider probe remains valid for its narrower scope; it does not clear the full source-chain blocker.
...
- The latest fresh Gateway resource attempt is intentionally retained as a provider-availability blocker: its document is `failed/unavailable` and initialization reports the same error. It must be rerun only after a reachable RAGFlow endpoint and `RAGFLOW_API_KEY` are supplied; this failure is not converted into a mock pass.
