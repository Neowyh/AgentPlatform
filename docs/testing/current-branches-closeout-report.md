...
Status: **本机集成候选仍待复核；正式收口未完成**. The candidate is fast-forwarded into `develop`, but the local Gateway is blocked by catalog/storage consistency and external acceptance gates remain open.
...
| TTFT diagnostic unit tests | Existing diagnostic tests remain available; the real fault-zeroing Agent run started with configured DeepSeek but ended incomplete after prolonged silence, so no performance conclusion is claimed |
...
- Local Gateway startup is currently blocked by `CatalogConsistencyError`: the database catalog points at modified custom Agent/Skill versions whose canonical storage hashes no longer match. The bundled seed restored bundled files but intentionally did not overwrite user-modified resources. This must be repaired or isolated before browser, Gate 7/8, and proxy-path TTFT runs.
