...
This file is for coding agents. If the iDeer repository is not already cloned and open, clone `https://github.com/bytedance/ideer.git` first, then continue from the repository root.

## Goal

Bootstrap a iDeer local development workspace on the user's machine with the least risky path available.
...
- The iDeer repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch iDeer.
...
- If the current directory is not the iDeer repository root, clone `https://github.com/bytedance/ideer.git` if needed, then change into the repository root.
- Confirm the current directory is the iDeer repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start iDeer
