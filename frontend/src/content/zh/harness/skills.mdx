...
每个技能位于 `resources/skills/`（或用户创建技能的 `resources/skills/`）下自己的子目录中。目录包含一个 `SKILL.md` 文件，定义技能的元数据、指令和工作流。
...
- **通过 iDeer 应用界面**：技能面板允许你切换技能的启用/禁用状态。
...
自定义 Agent 可以被限制为特定技能子集。在 Agent 的配置中（由统一资源目录管理，见 `resources/agents/<uuid>/` 的 agent-draft），设置 `skills` 列表：
...
iDeer 包含一个可选的**技能进化**功能，允许 Agent 在 `resources/skills/` 目录中自主创建和改进技能：
...
1. 在 `resources/skills/your-skill-name/` 下创建新目录
