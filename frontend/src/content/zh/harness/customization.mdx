---
title: 自定义与扩展
description: iDeer 的可插拔架构意味着系统的大多数部分都可以在不 fork 核心的情况下被替换或扩展。本页面列举了扩展点，并解释如何使用每一个。
---
...
iDeer 的可插拔架构意味着系统的大多数部分都可以在不 fork 核心的情况下被替换或扩展。本页面列举了扩展点，并解释如何使用每一个。
...
技能是最简单的扩展点。在 `resources/skills/your-skill-name/` 下创建一个目录，并在其中添加 `SKILL.md` 文件。技能将在下次 `load_skills()` 调用时自动被发现。
...
对于自定义后端，实现 LangGraph 的 `BaseCheckpointSaver` 接口，并在初始化 `iDeerClient` 时以编程方式配置它。
