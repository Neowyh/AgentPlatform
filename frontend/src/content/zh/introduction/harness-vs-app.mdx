---
title: Harness 与应用
description: iDeer 有两个紧密相关但服务于不同目的的层次：.
---
...
<Callout type="info" emoji="⚙️">
  iDeer 应用是构建在 iDeer Harness 之上的最佳实践 Super Agent 应用，而
  iDeer Harness 是构建自己 Agent 系统的 Python SDK 和运行时基础。
</Callout>

iDeer 有两个紧密相关但服务于不同目的的层次：

- **iDeer Harness** 是运行时基础层。
- **iDeer 应用** 是构建在该基础之上的最佳实践应用。

理解这一区别，能让其余文档更易于阅读。

## Harness 是运行时层

**iDeer Harness** 是用于构建和运行长时序 Agent 的可复用系统。

它提供：长时序任务的规划和执行、工具调用和沙箱执行、技能加载和上下文注入、记忆和跨会话持久化、子 Agent 协调、以及完整的配置和扩展系统。

Harness 是 Python 库和运行时引擎。你可以将它导入到自己的应用中，或者直接使用 iDeer 应用，后者已经为你完成了所有集成工作。

## 应用是产品层

**iDeer 应用**是一个完整的、可部署的产品，它将 Harness 功能封装成可用的体验：

- 基于浏览器的对话工作区（Next.js 前端）
- FastAPI Gateway，处理 API 操作
- nginx 反向代理，统一所有服务
- LangGraph 服务器，运行 iDeer Harness
...
使用 **Harness**。将其作为 Python 库导入，使用 `iDeerClient` API 发送消息和流式传输响应，并通过 `config.yaml` 配置行为。
