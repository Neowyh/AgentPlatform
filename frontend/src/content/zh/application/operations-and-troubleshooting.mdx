---
title: 运维与排障
description: 本页面涵盖运行 iDeer 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 iDeer 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

iDeer 应用在 `logs/` 目录中写入每个服务的日志：
...
```bash
# 检查线程用户数据目录是否存在且可写
ls -la backend/.ideer/threads/
```

**解决**：

1. 确保 `backend/.ideer/` 对运行 iDeer 的进程可写。
2. 在 Docker 部署中，验证卷挂载路径正确（`IDEER_ROOT` 设置为绝对路径）。
...
- **线程数据**：`backend/.ideer/threads/` — 每个线程的上传文件、输出和工作区文件
- **检查点**：取决于检查点器配置（SQLite：`backend/.ideer/checkpoints.db`，Redis：外部存储）
- **记忆**：`backend/.ideer/memory.json`（以及 `agent-memory/*/memory.json`）
- **自定义 Agent 配置**：统一资源目录（canonical）`backend/.ideer/resources/agents/*/`（draft + versions）
