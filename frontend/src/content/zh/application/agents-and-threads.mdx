---
title: Agent 与线程
description: 了解 iDeer 中 Agent 与线程的关系，以及如何管理自定义 Agent 和对话线程。
---
...
iDeer 允许你创建多个具有不同专业领域的自定义 Agent。每个 Agent 使用 iDeer Harness 相同的 Lead Agent 运行时，但具有不同的：
...
```bash
curl -X POST http://localhost:8001/api/resources \
...
如果派生的 slug 与现有 Agent 冲突，系统会建议一个唯一的替代名称（名称解析走 `/api/resources/aliases/agent/{slug}`）。

### Agent 存储

自定义 Agent 由统一资源目录（canonical）管理：定义（`SOUL.md` + `config.yaml`）以版本化草稿-发布（draft/publish）方式存储在 `resources/agents/<uuid>/versions/<version>`，通过 `PUT /api/resources/{id}/agent-draft` 更新、`POST /api/resources/{id}/publish` 发布，更改在下次 Agent 调用时自动加载。旧 `/api/agents` 路由已删除（404）。
...
iDeer 在每次 Agent 轮次后自动保存线程状态（如果配置了检查点器）。这允许：
...
每个线程在 `.ideer/threads/{thread_id}/` 下有一个专用目录：

```
.ideer/threads/{thread_id}/
  user-data/
    uploads/     ← 用户上传的文件
