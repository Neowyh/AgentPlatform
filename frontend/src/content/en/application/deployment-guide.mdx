...
description: "This guide covers all supported deployment methods for iDeer App: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for iDeer App: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run iDeer. All services run as native processes on your machine.
...
```bash
# Set the absolute path to your ideer repo root
export IDEER_ROOT=/path/to/ideer
...
# .env
OPENAI_API_KEY=sk-...
IDEER_ROOT=/absolute/path/to/ideer
...
Thread data is stored in `backend/.ideer/threads/`. In Docker deployments, this directory is bind-mounted into the gateway container.

To avoid data loss when containers are recreated:

1. Set `IDEER_ROOT` to the absolute repo root path (or a stable host path).
...
USERDATA_PVC_NAME=ideer-userdata-pvc
SKILLS_PVC_NAME=ideer-skills-pvc
```
...
iDeer App uses [Better Auth](https://www.better-auth.com/) for session management. In production:
