...
description: iDeer App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
---
...
iDeer App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
...
- **The Gateway API**: `POST /api/resources` with `resource_type: "agent"` and the agent definition.

The slug (`name`) is automatically derived from the `display_name` and must be unique. The system checks for conflicts and suggests a unique alternative (name resolution goes through `GET /api/resources/aliases/agent/{slug}`).

Agent definitions are managed by the unified resource catalog (canonical): `SOUL.md` + `config.yaml` are stored as versioned draft/publish content under `resources/agents/<uuid>/versions/<version>`, updated via `PUT /api/resources/{id}/agent-draft` and published via `POST /api/resources/{id}/publish`. The legacy `/api/agents` routes are removed (404).
...
```yaml
# agent definition in the resource catalog (agent-draft)
name: my-researcher
...
# checkpointer:
#   type: postgres
#   connection_string: postgresql://user:password@localhost:5432/ideer
```
...
<Callout type="info">
  ...
  <code>iDeerClient</code> in direct Python integrations.
</Callout>
...
```
backend/.ideer/threads/{thread_id}/user-data/
```

This directory is mounted into the sandbox as `/mnt/user-data/` for the agent to read and write.

In Docker deployments, this path is bind-mounted from the host so that thread data persists across container restarts. Set `IDEER_ROOT` to the absolute path of your ideer repository root to ensure the correct host path is used.
