...
description: This page covers day-to-day operational tasks and solutions to common problems when running iDeer App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running iDeer App.
...
When you pull a new version of iDeer, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
- Check that the storage path is writable: `ls -la backend/.ideer/`.
- Look for memory update errors in `logs/gateway.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.ideer/`:

```
backend/.ideer/
  memory.json           # global agent memory
  agent-memory/         # per-agent memory
  agents/               # custom agent configuration assets
...
In Docker deployments, the bind-mounted host path (`$IDEER_ROOT/backend/.ideer/`) is the source of truth — back up the host path.
