---
title: Configuration
description: iDeer App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
iDeer App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
Install Ollama provider: `cd backend && uv add 'ideer-harness[ollama]'`
...
Install: `cd backend && uv add 'ideer-harness[aio-sandbox]'`
...
tools:
  ...
  - use: ideer.community.ddg_search.tools:web_search_tool # default, no key required
...
By default, iDeer uses an SQLite checkpointer for thread state persistence:

```yaml
checkpointer:
  type: sqlite
  connection_string: checkpoints.db # stored in backend/.ideer/
```
