---
title: Memory
description: Memory is a runtime feature of the iDeer Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets iDeer carry useful information across sessions. The agent
...
Memory is a runtime feature of the iDeer Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  # Default: {base_dir}/memory.json  (resolves to backend/.ideer/memory.json)
  ...
  storage_class: ideer.agents.memory.storage.FileMemoryStorage
...
iDeer supports two levels of memory:
...
- **Per-agent memory**: stored at `{base_dir}/agent-memory/{agent_name}/memory.json`. Used when a custom agent is active, keeping that agent's learned knowledge separate from custom-agent configuration assets under `agents/`.
...
- Base directory: `backend/.ideer/`
- Global memory: `backend/.ideer/memory.json`
- Per-agent memory: `backend/.ideer/agent-memory/{agent_name}/memory.json`
