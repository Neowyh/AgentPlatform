...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. iDeer provides built-in
...
iDeer organizes tools into four categories:
...
Persists updates to the current custom agent's `SOUL.md` and `config.yaml`. Bound to the lead agent only when a custom agent is active (`agent_name` is set in the runtime context). Use this when the user asks the agent to refine its own description, personality, skill whitelist, tool-group whitelist, or default model — it resolves the agent's resource UUID from `agent_name` and writes through the unified resource catalog (canonical) as a draft + publish into the versioned store (`resources/agents/<uuid>/`), so the change is picked up automatically on the next user turn. Only the fields you explicitly pass are updated; omit a field to preserve its existing value. Pass `skills=[]` to disable all skills, or omit `skills` to keep the existing whitelist.
...
tools:
  ...
  - use: ideer.sandbox.tools:str_replace_tool
  - use: ideer.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
Install: `cd backend && uv add 'ideer-harness[tavily]'`
...
Install: `cd backend && uv add 'ideer-harness[exa]'`
...
Install: `cd backend && uv add 'ideer-harness[firecrawl]'`
...
tools:
  ...
  # Or use InfoQuest:
  # - use: ideer.community.infoquest.tools:image_search_tool
  #   api_key: $INFOQUEST_API_KEY
```
