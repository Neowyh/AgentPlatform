...
Each skill lives in its own subdirectory under `resources/skills/` (or `resources/skills/` for user-created skills). The directory contains a `SKILL.md` file that defines the skill's metadata, instructions, and workflow.
...
iDeer ships with the following public skills:
...
- **Through the iDeer App UI**: the skills panel lets you toggle skills on and off.
...
A custom agent can be restricted to a specific subset of skills. In the agent's config (managed by the unified resource catalog, see the agent-draft under `resources/agents/<uuid>/`), set a `skills` list:
...
iDeer includes an optional **skill evolution** feature that allows the agent to autonomously create and improve skills in the `resources/skills/` directory:
...
1. Create a new directory under `resources/skills/your-skill-name/`
