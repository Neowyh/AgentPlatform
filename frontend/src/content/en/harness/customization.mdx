---
title: Customization
description: iDeer's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
---
...
<Callout type="info" emoji="🔧">
  iDeer is designed to be adapted. You can extend agent behavior by writing
...
iDeer's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
...
```python
from ideer.agents.memory.storage import MemoryStorage
...
Skills are the easiest extension point. Create a directory under `resources/skills/your-skill-name/` with a `SKILL.md` file. The skill is discovered automatically on the next `load_skills()` call.
...
For custom backends, implement the LangGraph `BaseCheckpointSaver` interface and configure it programmatically when initializing the `iDeerClient`.
