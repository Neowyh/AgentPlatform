...
description: This tutorial shows you how to enable and use iDeer's memory system so the agent remembers important information about you across multiple sessions.
---
...
This tutorial shows you how to enable and use iDeer's memory system so the agent remembers important information about you across multiple sessions.
...
Memory is stored in `backend/.ideer/memory.json`:

```bash
cat backend/.ideer/memory.json
```
...
```
backend/.ideer/agent-memory/{agent_name}/memory.json
```

This keeps each agent's learned knowledge separate. The `agents/` directory is reserved for custom-agent configuration assets.

## Next steps

- [Deploy Your Own iDeer](/docs/tutorials/deploy-your-own-ideer)
