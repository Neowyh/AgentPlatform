      ...
      data: {
        label: (
          <div className="flex max-w-[200px] flex-col gap-2 text-left">
            ...
            {node.description && (
              <div className="text-muted-foreground line-clamp-3 text-xs leading-snug break-words">
                {node.description}
              </div>
            )}
            {badges.length > 0 && (
...
export function FaultTreeViewer({ content }: { content: string }) {
  ...
  return (
    <div className="flex size-full flex-col">
      <div className="border-border bg-background/80 grid shrink-0 grid-cols-2 gap-2 border-b p-3 text-sm md:grid-cols-4">
        <SummaryItem
          label="Bottom events"
      ...
      {result.diagnostics.length > 0 && (
        <div className="border-border bg-muted/40 shrink-0 border-b px-4 py-2 text-xs">
