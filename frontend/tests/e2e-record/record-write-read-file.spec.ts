 ...
 * call to IDEER_RECORD_OUT; this just needs to drive the flow and wait until
...
test("record write/read-file run through the real frontend", async ({
  ...
  const out = process.env.IDEER_RECORD_OUT;
  expect(out, "IDEER_RECORD_OUT must be set").toBeTruthy();
